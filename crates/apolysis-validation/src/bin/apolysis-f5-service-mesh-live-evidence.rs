...
use apolysis_validation::{evaluate_f5_service_mesh_live_evidence, F5ServiceMeshLiveEvidence};
...
fn main() {
    if let Err(error) = run(std::env::args().skip(1).collect()) {
        eprintln!("apolysis-f5-service-mesh-live-evidence: {error}");
        std::process::exit(2);
    }
}
...
fn run(args: Vec<String>) -> Result<(), String> {
    ...
    let evidence: F5ServiceMeshLiveEvidence = serde_json::from_str(&evidence_json)
...
fn parse_args(args: Vec<String>) -> Result<CliArgs, String> {
    match args.as_slice() {
        ...
        _ => Err("usage: apolysis-f5-service-mesh-live-evidence --evidence <path>".to_string()),
    }
}
