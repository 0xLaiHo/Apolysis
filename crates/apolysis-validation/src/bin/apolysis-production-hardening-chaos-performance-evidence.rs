...
use apolysis_validation::{
    evaluate_production_hardening_chaos_performance_evidence,
...
fn main() {
    if let Err(error) = run(std::env::args().skip(1).collect()) {
        eprintln!("apolysis-production-hardening-chaos-performance-evidence: {error}");
        std::process::exit(2);
    }
}
...
fn run(args: Vec<String>) -> Result<(), String> {
    ...
    let evidence: ProductionHardeningChaosPerformanceEvidence =
...
fn parse_args(args: Vec<String>) -> Result<CliArgs, String> {
    match args.as_slice() {
        ...
        _ => Err(
            "usage: apolysis-production-hardening-chaos-performance-evidence --evidence <path>"
                .to_string(),
        ),
    }
}
