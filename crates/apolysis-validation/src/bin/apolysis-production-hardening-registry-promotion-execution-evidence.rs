...
fn main() {
    if let Err(error) = run(std::env::args().skip(1).collect()) {
        eprintln!("apolysis-production-hardening-registry-promotion-execution-evidence: {error}");
        std::process::exit(2);
    }
}
...
fn run(args: Vec<String>) -> Result<(), String> {
    ...
    let evidence: ProductionHardeningRegistryPromotionExecutionEvidence =
...
fn parse_args(args: Vec<String>) -> Result<CliArgs, String> {
    match args.as_slice() {
        ...
        _ => Err(
            "usage: apolysis-production-hardening-registry-promotion-execution-evidence --evidence <path>"
                .to_string(),
        ),
    }
}
