...
fn main() {
    if let Err(error) = run(std::env::args().skip(1).collect()) {
        eprintln!("apolysis-production-hardening-operator-controller-evidence: {error}");
        std::process::exit(2);
    }
}
...
fn run(args: Vec<String>) -> Result<(), String> {
    ...
    let evidence: ProductionHardeningOperatorControllerEvidence =
        serde_json::from_str(&evidence_json).map_err(|error| {
            format!("failed to parse operator controller evidence JSON: {error}")
        })?;
...
fn parse_args(args: Vec<String>) -> Result<CliArgs, String> {
    match args.as_slice() {
        ...
        _ => Err(
            "usage: apolysis-production-hardening-operator-controller-evidence --evidence <path>"
                .to_string(),
        ),
    }
}
