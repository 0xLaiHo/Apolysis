...
use apolysis_validation::{
    evaluate_f3_block_enablement_policy, F3BlockEnablementRequest, F3BlockValidationGateReport,
};
...
fn main() {
    if let Err(error) = run(std::env::args().skip(1).collect()) {
        eprintln!("apolysis-f3-block-enablement-policy: {error}");
        std::process::exit(2);
    }
}
...
fn run(args: Vec<String>) -> Result<(), String> {
    ...
    let validation: F3BlockValidationGateReport = serde_json::from_str(&validation_input)
    ...
    let report = evaluate_f3_block_enablement_policy(validation, parsed.requests);
...
fn parse_validation_gate_arg(args: Vec<String>) -> Result<PathBuf, String> {
    match args.as_slice() {
        ...
        _ => Err("usage: apolysis-f3-block-enablement-policy --validation-gate <path>".to_string()),
    }
}
