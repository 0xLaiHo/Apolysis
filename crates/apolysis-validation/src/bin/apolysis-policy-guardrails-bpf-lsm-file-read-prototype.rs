...
fn main() {
    match run(std::env::args().skip(1).collect()) {
        ...
        Err(RunError::Fatal(error)) => {
            eprintln!("apolysis-policy-guardrails-bpf-lsm-file-read-prototype: {error}");
            std::process::exit(2);
        }
    }
}
...
fn run(args: Vec<String>) -> Result<(), RunError> {
    ...
    let prereq = evaluate_policy_guardrails_bpf_lsm_prototype_prerequisites(environment);
    if !prereq.passed {
...
fn usage() -> String {
    "usage: apolysis-policy-guardrails-bpf-lsm-file-read-prototype --bpf-object <path> --target-path <path>"
        .to_string()
}

fn detect_environment(object_path: &Path) -> PolicyGuardrailsBpfLsmPrototypeEnvironment {
