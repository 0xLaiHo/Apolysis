...
use apolysis_validation::{
    default_f2_performance_budgets, evaluate_performance_gate, PerformanceBudget, PerformanceSample,
};
...
fn main() {
    if let Err(error) = run() {
        eprintln!("apolysis-f2-performance: {error}");
        std::process::exit(2);
    }
}
...
fn run() -> Result<(), String> {
    ...
    let (budgets, samples) = match parsed {
        PerformanceReportInput::Samples(samples) => (default_f2_performance_budgets(), samples),
