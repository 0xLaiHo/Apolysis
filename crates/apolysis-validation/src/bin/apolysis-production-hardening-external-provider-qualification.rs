...
use apolysis_validation::{
    ...
    ProductionHardeningExternalProviderQualificationFailure,
};
...
fn main() {
    if let Err(error) = run(std::env::args().skip(1).collect()) {
        eprintln!("apolysis-production-hardening-external-provider-qualification: {error}");
        std::process::exit(2);
    }
}
...
fn run(args: Vec<String>) -> Result<(), String> {
    ...
    let bundle: ProductionHardeningExternalProviderQualificationBundle =
        serde_json::from_str(&bundle_json).map_err(|error| {
            format!("failed to parse external provider qualification bundle JSON: {error}")
        })?;
    let mut report =
        evaluate_production_hardening_external_provider_qualification_bundle(bundle.clone());
...
fn usage() -> String {
    "usage: apolysis-production-hardening-external-provider-qualification --bundle <path> [--bundle-root <path>]"
        .to_string()
}
...
fn verify_retained_artifacts(
    ...
    root: &Path,
) -> Vec<ProductionHardeningExternalProviderQualificationFailure> {
