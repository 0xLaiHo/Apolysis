...
use apolysis_validation::{
    evaluate_f5_release_promotion_policy, F5ReleasePromotionPolicyEvidence,
...
fn main() {
    if let Err(error) = run(std::env::args().skip(1).collect()) {
        eprintln!("apolysis-f5-release-promotion-policy: {error}");
        std::process::exit(2);
    }
}
...
fn run(args: Vec<String>) -> Result<(), String> {
    ...
    let request: F5ReleasePromotionRequest = parse_json(
        &request_bytes,
        &format!("promotion request {}", args.request.display()),
    )?;
    let evidence = F5ReleasePromotionPolicyEvidence {
...
fn usage() -> String {
    "usage: apolysis-f5-release-promotion-policy --release-manifest <path> --registry-attachment <path> --archive-manifest <path> --request <path>".to_string()
}
