...
use apolysis_validation::{
    evaluate_runtime_guardrails_live_runtime_evidence_bundle,
    RuntimeGuardrailsLiveRuntimeEvidenceBundleRequest, RuntimeGuardrailsRuntimeGuardrailTarget,
    VisibilityReport, VisibilityTarget,
};

#[test]
fn runtime_guardrails_live_runtime_evidence_bundle_requires_matrix_artifacts_and_visibility_gate() {
    let root = test_root("runtime-guardrails-live-runtime-evidence-pass");
    create_matrix_artifacts(&root);
    let request = bundle_request(&root, matrix_evidence_source(&root));

    let report = evaluate_runtime_guardrails_live_runtime_evidence_bundle(request);
    ...
    let kata = matrix
        .runtimes
        .iter()
        .find(|entry| entry.runtime == RuntimeGuardrailsRuntimeGuardrailTarget::Kata)
        .expect("kata row");
    assert!(kata
        .notify
        .evidence_ids
...
#[test]
fn runtime_guardrails_live_runtime_evidence_bundle_fails_closed_without_artifact_dir() {
    let root = test_root("runtime-guardrails-live-runtime-evidence-missing-artifacts");
    let request = bundle_request(&root, matrix_evidence_source(&root));
...
#[test]
fn runtime_guardrails_live_runtime_evidence_bundle_rejects_visibility_source_mismatch() {
    let root = test_root("runtime-guardrails-live-runtime-evidence-source-mismatch");
    create_matrix_artifacts(&root);
    let request = bundle_request(
        &root,
        "scripts/test-runtime-foundation-runtime-adapter-matrix.sh artifacts=/tmp/other",
    );
...
) -> RuntimeGuardrailsLiveRuntimeEvidenceBundleRequest {
    RuntimeGuardrailsLiveRuntimeEvidenceBundleRequest {
...
fn matrix_evidence_source(root: &Path) -> String {
    format!(
        "scripts/test-runtime-foundation-runtime-adapter-matrix.sh artifacts={}",
        root.display()
    )
}

fn failure_text(
    failures: &[apolysis_validation::RuntimeGuardrailsLiveRuntimeEvidenceBundleFailure],
) -> String {
    failures
        .iter()
