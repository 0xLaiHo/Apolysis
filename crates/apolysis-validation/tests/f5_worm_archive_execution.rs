...
use apolysis_validation::{
    evaluate_f5_worm_archive_execution_evidence, F5WormArchiveExecutionEvidence,
    F5WormArchiveExecutionSource, F5WormProvider, F5WormRetentionMode,
};
...
#[test]
fn f5_worm_archive_execution_accepts_live_s3_object_lock_api_evidence() {
    let report = evaluate_f5_worm_archive_execution_evidence(worm_api_execution_evidence());
    ...
    assert_eq!(
        approval.object_key,
        "releases/apolysis/f5-release-manifest.json"
    );
    assert_eq!(approval.object_version_id, "f5-version-0001");
    assert_eq!(approval.retention_mode, F5WormRetentionMode::Compliance);
...
fn f5_worm_archive_execution_rejects_fixture_or_mutable_api_evidence() {
    ...
    evidence.provider = F5WormProvider::LocalFilesystem;
    ...
    evidence.retention_mode = F5WormRetentionMode::Governance;
...
fn worm_api_execution_evidence() -> F5WormArchiveExecutionEvidence {
    let observed_at = 1_782_259_200_000;
    F5WormArchiveExecutionEvidence {
        evidence_id: "f5-worm-api-execution-20260624".to_string(),
        source: F5WormArchiveExecutionSource::LiveProvider,
        ...
        object_key: "releases/apolysis/f5-release-manifest.json".to_string(),
        object_version_id: "f5-version-0001".to_string(),
        ...
        audit_log_ref: "s3://apolysis-prod-release-archive/.audit/f5-worm-api-execution"
            .to_string(),
