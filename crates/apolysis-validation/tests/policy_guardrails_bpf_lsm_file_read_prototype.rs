...
use apolysis_validation::{
    evaluate_policy_guardrails_bpf_lsm_prototype_prerequisites,
    PolicyGuardrailsBpfLsmPrototypeEnvironment,
};

#[test]
fn policy_guardrails_bpf_lsm_prerequisites_accept_live_capable_environment() {
    let report = evaluate_policy_guardrails_bpf_lsm_prototype_prerequisites(capable_environment());
...
#[test]
fn policy_guardrails_bpf_lsm_prerequisites_fail_closed_without_active_lsm_or_privilege() {
    let mut environment = capable_environment();
    environment.bpf_lsm_active = false;
    environment.privileged_for_bpf = false;

    let report = evaluate_policy_guardrails_bpf_lsm_prototype_prerequisites(environment);
...
#[test]
fn policy_guardrails_bpf_lsm_prerequisites_fail_closed_without_btf_or_object() {
    let mut environment = capable_environment();
...
fn capable_environment() -> PolicyGuardrailsBpfLsmPrototypeEnvironment {
    PolicyGuardrailsBpfLsmPrototypeEnvironment {
        linux: true,
        btf_available: true,
