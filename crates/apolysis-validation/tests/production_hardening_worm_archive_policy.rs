...
#[test]
fn production_hardening_worm_archive_policy_accepts_external_object_lock_compliance_archive() {
    let report = evaluate_production_hardening_worm_archive_policy(s3_object_lock_policy());
...
#[test]
fn production_hardening_worm_archive_policy_rejects_mutable_or_underprotected_archive() {
    let mut policy = s3_object_lock_policy();
    policy.provider = ProductionHardeningWormProvider::LocalFilesystem;
    ...
    policy.retention_mode = ProductionHardeningWormRetentionMode::Governance;
...
fn s3_object_lock_policy() -> ProductionHardeningWormArchivePolicy {
    let requested_at = 1_782_259_200_000;
    ProductionHardeningWormArchivePolicy {
        policy_id: "production-hardening-prod-worm-archive".to_string(),
