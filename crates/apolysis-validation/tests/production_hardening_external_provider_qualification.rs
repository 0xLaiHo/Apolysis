...
use apolysis_validation::{
    evaluate_production_hardening_external_provider_qualification_bundle,
    ProductionHardeningExternalProviderQualificationBundle,
    ProductionHardeningExternalProviderQualificationEntry,
    ProductionHardeningExternalProviderQualificationRequirement,
    ProductionHardeningExternalProviderQualificationSource,
};
...
#[test]
fn production_hardening_external_provider_qualification_accepts_bundle_with_r2_and_dockerhub_evidence(
) {
    let report = evaluate_production_hardening_external_provider_qualification_bundle(
    ...
    assert_eq!(
        approval.bundle_id,
        "production-hardening-external-provider-qualification-20260624"
    );
...
#[test]
fn production_hardening_external_provider_qualification_accepts_vultr_vke_istio_provider_evidence()
{
    ...
    let report = evaluate_production_hardening_external_provider_qualification_bundle(bundle);
...
#[test]
fn production_hardening_external_provider_qualification_rejects_local_or_incomplete_bundles() {
    let mut bundle = qualification_bundle();
    bundle.source = ProductionHardeningExternalProviderQualificationSource::Fixture;
    bundle.operator_approved = false;
    bundle.generated_at_unix_ms = 0;
    bundle.entries = vec![ProductionHardeningExternalProviderQualificationEntry {
        requirement: ProductionHardeningExternalProviderQualificationRequirement::CloudKmsOrExternalHsmSigning,
        provider: "softhsm".to_string(),
        provider_control_plane: "local workstation".to_string(),
        evidence_ref: "target/production-hardening-signing-execution/local.json".to_string(),
...
#[test]
fn production_hardening_external_provider_qualification_cli_verifies_retained_artifact_files() {
    let root = fresh_temp_dir("production-hardening-external-provider-artifacts-pass");
    ...
    let output = Command::new(env!(
        "CARGO_BIN_EXE_apolysis-production-hardening-external-provider-qualification"
    ))
    .arg("--bundle")
    .arg(&bundle_path)
    .arg("--bundle-root")
...
#[test]
fn production_hardening_external_provider_qualification_cli_rejects_missing_or_mismatched_artifacts(
) {
    let root = fresh_temp_dir("production-hardening-external-provider-artifacts-fail");
...
#[cfg(unix)]
#[test]
fn production_hardening_external_provider_qualification_cli_rejects_artifacts_that_escape_bundle_root(
) {
    use std::os::unix::fs::symlink;

    let root = fresh_temp_dir("production-hardening-external-provider-artifacts-escape");
    let outside = fresh_temp_dir("production-hardening-external-provider-artifacts-outside");
...
fn qualification_bundle() -> ProductionHardeningExternalProviderQualificationBundle {
    ProductionHardeningExternalProviderQualificationBundle {
        bundle_id: "production-hardening-external-provider-qualification-20260624".to_string(),
        source: ProductionHardeningExternalProviderQualificationSource::EvidenceBundle,
        ...
        entries: vec![
            qualification_entry(
                ...
                "aws_kms",
                "aws-kms:us-west-2:alias/apolysis-production-hardening-release",
            ),
            qualification_entry(
                ProductionHardeningExternalProviderQualificationRequirement::CloudWormObjectLockArchive,
                "cloudflare_r2_bucket_lock",
                "cloudflare-r2:e85b6fa3634dc882cfbd2188361fb37e:apolysis-production-hardening-worm-1782254413912",
            ),
            qualification_entry(
                ProductionHardeningExternalProviderQualificationRequirement::CloudRegistryPromotionRetention,
                "docker_hub",
                "docker-hub:devlaiho:apolysis-production-hardening-registry",
            ),
            qualification_entry(
                ProductionHardeningExternalProviderQualificationRequirement::ManagedServiceMesh,
