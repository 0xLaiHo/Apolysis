...
use apolysis_validation::{
    evaluate_f5_operator_controller_evidence, F5OperatorControllerEvidence,
    F5OperatorControllerEvidenceSource, F5OperatorControllerProvider,
    F5OperatorControllerRbacScope,
};

#[test]
fn f5_operator_controller_accepts_live_reconciliation_evidence() {
    let report = evaluate_f5_operator_controller_evidence(operator_controller_evidence());

    assert!(report.passed, "{:#?}", report.failures);
    let approval = report.approval.expect("operator controller approval");
    assert_eq!(approval.cluster_name, "mactavish-k3s");
    assert_eq!(approval.namespace, "apolysis-f5-operator");
    assert_eq!(
        approval.provider,
        F5OperatorControllerProvider::KubernetesController
    );
...
#[test]
fn f5_operator_controller_rejects_static_or_unbounded_controller_claims() {
    let mut evidence = operator_controller_evidence();
    evidence.source = F5OperatorControllerEvidenceSource::Fixture;
    evidence.provider = F5OperatorControllerProvider::StaticManifest;
    ...
    evidence.rbac_scope = F5OperatorControllerRbacScope::ClusterAdmin;
    ...
    let report = evaluate_f5_operator_controller_evidence(evidence);
...
fn operator_controller_evidence() -> F5OperatorControllerEvidence {
    F5OperatorControllerEvidence {
        evidence_id: "f5-operator-controller-20260624".to_string(),
        source: F5OperatorControllerEvidenceSource::LiveCluster,
        provider: F5OperatorControllerProvider::KubernetesController,
        cluster_name: "mactavish-k3s".to_string(),
        namespace: "apolysis-f5-operator".to_string(),
        crd_name: "apolysisproductionconfigs.apolysis.dev".to_string(),
        custom_resource_name: "platform-production".to_string(),
        controller_deployment: "apolysis-f5-operator-controller".to_string(),
        controller_service_account: "apolysis-f5-operator-controller".to_string(),
        controller_desired_replicas: 2,
        controller_ready_replicas: 2,
        leader_election_lease: "apolysis-f5-operator-leader".to_string(),
        lease_holder_identity: "apolysis-f5-operator-controller-7d6c5d9c8c-kj4p7".to_string(),
        rbac_scope: F5OperatorControllerRbacScope::NamespaceScoped,
