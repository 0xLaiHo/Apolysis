...
use apolysis_validation::{
    evaluate_f5_chaos_performance_evidence, F5ChaosAction, F5ChaosPerformanceEvidence,
    F5ChaosPerformanceProvider, F5ChaosPerformanceSource,
};

#[test]
fn f5_chaos_performance_accepts_live_k3s_scale_and_recovery_evidence() {
    let report = evaluate_f5_chaos_performance_evidence(chaos_performance_evidence());

    assert!(report.passed, "{:#?}", report.failures);
    let approval = report.approval.expect("chaos performance approval");
    assert_eq!(approval.provider, F5ChaosPerformanceProvider::K3s);
...
#[test]
fn f5_chaos_performance_rejects_fixture_or_under_scaled_unmeasured_runs() {
    let mut evidence = chaos_performance_evidence();
    evidence.source = F5ChaosPerformanceSource::Fixture;
    evidence.provider = F5ChaosPerformanceProvider::Fixture;
...
fn chaos_performance_evidence() -> F5ChaosPerformanceEvidence {
    F5ChaosPerformanceEvidence {
        evidence_id: "f5-chaos-performance-20260624".to_string(),
        source: F5ChaosPerformanceSource::LiveCluster,
        provider: F5ChaosPerformanceProvider::K3s,
        cluster_name: "mactavish-k3s".to_string(),
        namespace: "apolysis-f5-chaos-performance".to_string(),
        ...
        pod_churn_deleted: 6,
        chaos_actions: vec![
            F5ChaosAction::PodDelete,
            F5ChaosAction::DeploymentSelfHealing,
            F5ChaosAction::MetricsScrape,
        ],
