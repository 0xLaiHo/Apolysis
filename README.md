...
cluster readiness gate for subsequent live Kubernetes validation, and a final
provider readiness audit that fail-closes when required signing or managed
service-mesh evidence is missing and rejects accepted-looking fixtures without
live-provider evidence source, plus a manual GitHub Actions workflow for running
the remaining live provider evidence gates with repository secrets, and a final
provider bundle environment preparation helper with workflow bundle assembly,
retained provider artifact package handoff, and retained provider workflow
alternatives.
...
provider readiness audit with live-provider fixture rejection, a manual final
provider evidence workflow, and a final provider bundle environment preparation
