...
that AWS KMS bootstrap before F5.25 signing, a final provider completion gate
that binds that readiness audit to a passing final external provider bundle, a
manual GitHub Actions workflow for running the remaining live provider evidence
gates with repository secrets, and a final provider bundle environment
preparation helper with workflow bundle assembly, retained provider artifact
package handoff, and retained provider workflow alternatives.
...
handoff to live signing with workflow bootstrap orchestration. The remaining
F5 production-provider gap is real cloud KMS or external hardware HSM signing
evidence.
