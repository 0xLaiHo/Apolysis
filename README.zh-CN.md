...
| F5 | Production Hardening：DaemonSet privilege budget、multi-tenant storage/query/retention metadata、mTLS/RBAC、signed artifacts、SBOM/provenance、KMS/HSM signing profile validation 和 PKCS#11 signing execution、Helm、registry/archive/promotion/WORM policy 与 API execution validation（包含 live OCI promotion）、service-mesh identity/live handshake validation、operator、大规模 chaos/performance testing | 🚧 **进行中** |
