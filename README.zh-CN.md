...
retention/access control 的 release promotion policy validation。
...
| F5 | Production Hardening：DaemonSet privilege budget、multi-tenant storage/query/retention metadata、mTLS/RBAC、signed artifacts、SBOM/provenance、Helm、registry/archive/promotion policy validation、service-mesh identity policy、operator、大规模 chaos/performance testing | 🚧 **进行中** |
