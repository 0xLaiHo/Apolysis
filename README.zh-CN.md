...
当 coding agent 已经直接运行在 Linux host 上，而 operator 希望为后续 tool actions
采集独立 evidence 时，使用这个 live observer 模式。下面示例来自一次本地 Codex
审计：Node wrapper PID 是 `1419187`，Codex vendor process PID 是 `1419194`，
Apolysis 在 agent 执行只读命令时观测 vendor process。
...
在 observer 运行期间，让被审计的 agent session 执行 workload：
...
`--scope-pid` 应使用当前 vendor process PID；上一轮 session 的 PID 通常已经失效。
对已经运行中的 agent，挂到长期存在的 vendor process 可以从 host 视角捕获 Codex
runtime 活动，例如代理连接和短生命周期进程退出。
