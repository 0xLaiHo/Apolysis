...
policy rendering validation，以及 daemon API 中的 tenant-scoped query/retention
metadata。
...
| F5 | Production Hardening：DaemonSet privilege budget、multi-tenant storage/query metadata、mTLS/RBAC、signed artifacts、SBOM/provenance、Helm、registry/archive validation、service-mesh identity policy、operator、大规模 chaos/performance testing | 🚧 **进行中** |
