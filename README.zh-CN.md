...
M5 阶段实现的是第三层的首个 policy-feedback 闭环。它会记录本地 session、process-tree 归属、Docker runtime metadata、fixture ring-buffer 事件、raw kernel event、归一化副作用事件、policy violation、降级 metadata、feedback 文件和 JSONL timeline。仓库现在包含 eBPF observer ABI 与 attach point skeleton；BPF-LSM 执行控制已经建模并做能力门控，但真实内核阻断目前尚未启用。
...
crates/
  ...
  apolysis-policy/  YAML/JSON policy parser 与 M5 决策逻辑。
