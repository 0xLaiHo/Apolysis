...
M2 阶段实现的是第三层的本地 runner 基础，当前只包含 Rust audit-mode 组件。它会记录本地 session、process-tree 归属、runtime metadata、超时 notify 和 JSONL timeline。内核 eBPF 采集和 BPF-LSM 执行控制已经纳入计划，但目前尚未启用。
