...
retention metadata gate 和 immutable registry retention metadata gate；正式 regulated
release 前仍需要 live provider readback 和 managed-mesh evidence decision。
...
| F6 | Regulated Release and Evidence Integrity：aggregate audit gate、provider execution planning、provider artifact import、final closure orchestration、retained live AWS KMS signing evidence、F6 evidence package gate、retained evidence package handoff、external WORM/object-lock retention metadata、immutable registry retention metadata、managed-mesh evidence decision，以及 final external-provider bundle closure | 🚧 **进行中** |
