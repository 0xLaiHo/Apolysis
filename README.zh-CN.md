...
竞态。同级的有界 direct-mTLS/PostgreSQL 门禁现已覆盖 transaction wait 或一次真实内部
transaction retry 跨过已接受 finalization deadline 或最后一个 lease expiry 时的 exact replay
对 novel ingest。Exact replay 保持稳定；novel work 失败关闭且不产生新的 event、operation
或 replay effect，只允许惰性提交一组匹配的 incomplete transition/outbox。它不代表完整
mixed lifecycle/retry matrix 已通过。更广的进程死亡与 network pre-commit matrix、
join/bind 及 staggered multi-lease 边界场景、load、replication、failover、backup/restore
与高可用仍未验证；production KMS integration、database RLS
