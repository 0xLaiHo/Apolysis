...
M7 阶段实现的是第三层的首个强隔离可观测性验证路径。它会记录本地 session、process-tree 归属、Docker runtime metadata、Kubernetes Pod metadata、fixture ring-buffer 事件、raw kernel event、归一化副作用事件、policy violation、降级 metadata、feedback 文件、visibility assessment 和 JSONL timeline。仓库现在包含 eBPF observer ABI 与 attach point skeleton；BPF-LSM 执行控制已经建模并做能力门控，但真实内核阻断目前尚未启用。
...
验证器会对比 Docker default、Docker+gVisor、Kubernetes+gVisor、Kubernetes+Kata 和 Firecracker boundary 场景下的 host-side observer fixture。它会记录 host 语义是否折叠、是否需要 runtime metadata、是否需要 guest-side collector。详见 [docs/visibility-validation.md](docs/visibility-validation.md)。
...
docs/
  visibility-validation.md 强隔离可观测性结论。
ebpf/
