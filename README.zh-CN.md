...
均已完成。F3 已包含 enforcement capability matrix、validated-enable/report
gate、live seccomp 与 BPF-LSM pre-operation `Block` prototype evidence、
operator-approved opt-in enablement、持久化 approval/rollback audit record，以及
消费 approved enablement report 的 local seccomp execution gate。
