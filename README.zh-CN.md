...
F0（M1-M7）完成了第三层的首个 PoC baseline。它会记录本地 session、process-tree 归属、Docker runtime metadata、Kubernetes Pod metadata、fixture ring-buffer 事件、raw kernel event、归一化副作用事件、policy violation、降级 metadata、feedback 文件、visibility assessment 和 JSONL timeline。F1 已实现带 session scope 的 live audit-only eBPF observer，包括 CO-RE build、Aya loader、process/file/network 事件、loss diagnostics 和持久化前脱敏，并已完成特权 live-host 验证。F3 在 production-facing kernel blocking 默认关闭的前提下，完成了 narrow local seccomp 和 BPF-LSM pre-operation block prototype validation，并加入 operator-approved enablement 与 rollback audit record。F4 已完成 runtime adapter depth：它区分已支持的 audit/review/kill 路径、本机 block prototype、metadata-only 强隔离声明、VM-backed runtime 的 boundary-only 能力，以及已验证的 Docker/containerd/Kubernetes adapter evidence；同时新增 live gVisor runsc/sentry/gofer metadata evidence、Kubernetes Agent Sandbox metadata evidence、Kata boundary-only evidence 和 live-runtime evidence bundle gate，用于把 F4 claim 绑定到保留的 F2 runtime adapter matrix artifact。F5 现在被定义为 Production Runtime Accountability Beta，并已在这个边界内完成：有边界的 Kubernetes deployment、live k3s/VKE 验证、operational metrics、failure recovery、tenant-aware retention/query、runtime metadata correlation，以及可用时的 service-mesh identity evidence。旧 F5 轨道中已经实现的 provider signing 与 external-provider bundle 工作被保留为 F6 基础。F6 Regulated Release and Evidence Integrity 已从 aggregate audit gate、provider execution planning gate、provider artifact import gate、final provider closure gate 和 signing evidence gate 开始，复用历史 provider gates 且不重命名 artifact contracts；它现在已有 retained live AWS KMS signing evidence、封装 final external-provider bundle builder 的 F6 evidence package gate、retained evidence package handoff gate、external WORM/object-lock retention metadata gate、immutable registry retention metadata gate、managed-mesh evidence decision gate，以及 live provider readback evidence gate；正式 regulated release 前仍需要最终签核。
...
和 F4 Runtime Adapter Depth 均已完成。F5 Production Runtime Accountability Beta
已经在 production-adjacent runtime accountability 边界内完成：有边界的
Kubernetes deployment、live k3s/VKE 验证、operational metrics、recovery、
tenant-aware retention/query、runtime metadata correlation，以及可用时的
service-mesh identity evidence。F6 Regulated Release and Evidence Integrity 已从
aggregate audit gate、provider execution planning gate、provider artifact import gate、
final provider closure gate 和 signing evidence gate 开始。它现在已有 retained live AWS KMS
signing evidence、封装 final external-provider bundle builder 的 F6 evidence package
gate、retained evidence package handoff gate，以及 external WORM/object-lock
retention metadata gate、immutable registry retention metadata gate，以及 managed-mesh
evidence decision gate 和 live provider readback evidence gate；正式 regulated
release 前仍需要最终签核。
...
| F5 | Production Runtime Accountability Beta：有边界的 Kubernetes deployment、live k3s/VKE 验证、metrics、recovery、tenant-aware retention/query、runtime metadata correlation，以及可用时的 service-mesh identity evidence | ✅ **已完成** |
| F6 | Regulated Release and Evidence Integrity：aggregate audit gate、provider execution planning、provider artifact import、final closure orchestration、retained live AWS KMS signing evidence、F6 evidence package gate、retained evidence package handoff、external WORM/object-lock retention metadata、immutable registry retention metadata、managed-mesh evidence decision gate、live provider readback evidence gate，以及 final external-provider bundle closure | 🚧 **进行中** |
