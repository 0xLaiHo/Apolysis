...
output_dir="${APOLYSIS_F5_PROVIDER_WORKFLOW_ARTIFACT_IMPORT_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/f5-provider-workflow-artifact-import.XXXXXX")}"
mkdir -p "$output_dir"
output_dir="$(cd "$output_dir" && pwd)"

report="$output_dir/apolysis-f5-provider-workflow-artifact-import-report.json"
import_root="$output_dir/provider-artifacts"
bundle_env_dir="$output_dir/final-provider-bundle-env"
require_ready="${APOLYSIS_REQUIRE_F5_PROVIDER_WORKFLOW_ARTIFACT_IMPORT:-0}"
...
mode = os.environ.get("APOLYSIS_F5_PROVIDER_WORKFLOW_ARTIFACT_IMPORT_MODE", "audit")
download_confirmed = os.environ.get("APOLYSIS_CONFIRM_F5_PROVIDER_WORKFLOW_ARTIFACT_DOWNLOAD", "0") == "1"
repo = origin_repo()
run_id = os.environ.get("APOLYSIS_F5_PROVIDER_WORKFLOW_RUN_ID", "")
local_artifact_root = os.environ.get("APOLYSIS_F5_PROVIDER_WORKFLOW_ARTIFACT_ROOT", "")
retained_package = os.environ.get("APOLYSIS_F5_RETAINED_PROVIDER_ARTIFACT_PACKAGE", "")
retained_package_sha256 = os.environ.get("APOLYSIS_F5_RETAINED_PROVIDER_ARTIFACT_PACKAGE_SHA256", "")
run_final_bundle = os.environ.get("APOLYSIS_RUN_F5_PROVIDER_WORKFLOW_FINAL_BUNDLE", "0") == "1"
...
workflow_contract = {
    ...
    "aws_kms_artifact": "f5-aws-kms-signing-evidence" in workflow_text,
    "final_bundle_artifact": "f5-final-external-provider-bundle" in workflow_text,
}
...
if mode not in {"audit", "download"}:
    missing_requirements.append("APOLYSIS_F5_PROVIDER_WORKFLOW_ARTIFACT_IMPORT_MODE_audit_or_download")
...
if mode == "download":
    if not valid_run_id(run_id):
        missing_requirements.append("APOLYSIS_F5_PROVIDER_WORKFLOW_RUN_ID")
    if not download_confirmed:
        missing_requirements.append("APOLYSIS_CONFIRM_F5_PROVIDER_WORKFLOW_ARTIFACT_DOWNLOAD")
...
if artifact_roots:
    env = os.environ.copy()
    env["APOLYSIS_F5_PROVIDER_ARTIFACT_ROOT"] = ":".join(str(root) for root in artifact_roots)
    env["APOLYSIS_F5_FINAL_PROVIDER_BUNDLE_ENV_OUTPUT_DIR"] = str(bundle_env_dir)
    env["APOLYSIS_RUN_F5_FINAL_BUNDLE"] = "1" if run_final_bundle else "0"
    rc, output = run([str(repo_root / "scripts/prepare-f5-final-provider-bundle-env.sh")], env=env)
    bundle_env_exit_code = rc
    bundle_env_output_hint = output[:4000]
    bundle_env_report_path = bundle_env_dir / "apolysis-f5-final-provider-bundle-env-report.json"
...
report = {
    ...
    "notes": [
        ...
        "Download mode requires APOLYSIS_CONFIRM_F5_PROVIDER_WORKFLOW_ARTIFACT_DOWNLOAD=1 before gh run download is executed.",
        "Imported artifacts are handed to scripts/prepare-f5-final-provider-bundle-env.sh through APOLYSIS_F5_PROVIDER_ARTIFACT_ROOT.",
    ...
    "next_commands": {
        "list_workflow_runs": f"gh run list --repo {repo or '<owner>/<repo>'} --workflow f5-final-provider-evidence.yml",
        "download_workflow_artifacts": f"APOLYSIS_F5_PROVIDER_WORKFLOW_ARTIFACT_IMPORT_MODE=download APOLYSIS_CONFIRM_F5_PROVIDER_WORKFLOW_ARTIFACT_DOWNLOAD=1 APOLYSIS_F5_PROVIDER_WORKFLOW_RUN_ID=<run-id> APOLYSIS_REQUIRE_F5_PROVIDER_WORKFLOW_ARTIFACT_IMPORT=1 ./scripts/test-f5-provider-workflow-artifact-import.sh",
        "audit_local_artifacts": "APOLYSIS_F5_PROVIDER_WORKFLOW_ARTIFACT_ROOT=<artifact-root> APOLYSIS_REQUIRE_F5_PROVIDER_WORKFLOW_ARTIFACT_IMPORT=1 ./scripts/test-f5-provider-workflow-artifact-import.sh",
        "audit_retained_package": "APOLYSIS_F5_RETAINED_PROVIDER_ARTIFACT_PACKAGE=<tar.gz> APOLYSIS_F5_RETAINED_PROVIDER_ARTIFACT_PACKAGE_SHA256=<sha256> APOLYSIS_REQUIRE_F5_PROVIDER_WORKFLOW_ARTIFACT_IMPORT=1 ./scripts/test-f5-provider-workflow-artifact-import.sh",
    },
...
if require_ready and not ready:
    print(f"apolysis-f5: provider workflow artifact import failed closed ({report_path})", file=sys.stderr)
    print("missing requirements: " + ", ".join(missing_requirements), file=sys.stderr)
    raise SystemExit(1)
PY

cat <<EOF
apolysis-f5: provider workflow artifact import audit written ($output_dir)
APOLYSIS_F5_PROVIDER_WORKFLOW_ARTIFACT_IMPORT_REPORT=$report
EOF
