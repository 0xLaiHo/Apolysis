...
output_dir="${APOLYSIS_PRODUCTION_HARDENING_WORM_ARCHIVE_POLICY_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/production-hardening-worm-archive-policy.XXXXXX")}"
mkdir -p "$output_dir"
...
pass_policy="$output_dir/apolysis-production-hardening-worm-archive-policy.json"
fail_policy="$output_dir/apolysis-production-hardening-worm-archive-policy-fail.json"
pass_report="$output_dir/apolysis-production-hardening-worm-archive-policy-report.json"
fail_report="$output_dir/apolysis-production-hardening-worm-archive-policy-fail-report.json"

cat >"$pass_policy" <<'JSON'
{
  "policy_id": "production-hardening-prod-worm-archive",
...
{
  "policy_id": "production-hardening-mutable-archive",
...
cargo run -q -p apolysis-validation --bin apolysis-production-hardening-worm-archive-policy -- \
...
if cargo run -q -p apolysis-validation --bin apolysis-production-hardening-worm-archive-policy -- \
    --policy "$fail_policy" >"$fail_report"; then
    echo "apolysis-production-hardening: mutable or local WORM archive policy unexpectedly passed" >&2
    exit 1
fi
...
printf 'apolysis-production-hardening: external WORM archive policy gate passed (%s)\n' "$output_dir"
