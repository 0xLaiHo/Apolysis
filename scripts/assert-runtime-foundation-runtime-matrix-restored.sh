...
if [[ -z "$artifact_dir" || ! -d "$artifact_dir" ]]; then
    cat >&2 <<'EOF'
apolysis-runtime_foundation: restore assertion requires APOLYSIS_RUNTIME_ADAPTER_MATRIX_OUTPUT_DIR
...
if [[ ! -s "$manifest" ]]; then
    echo "apolysis-runtime_foundation: missing backup manifest: $manifest" >&2
    exit 2
fi
...
if failures:
    for failure in failures:
        print(f"apolysis-runtime_foundation: {failure}", file=sys.stderr)
    sys.exit(1)
PY

echo "apolysis-runtime_foundation: runtime matrix restore matches backup manifest"
