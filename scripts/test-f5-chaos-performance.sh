...
confirm="${APOLYSIS_CONFIRM_F5_CHAOS_PERFORMANCE:-0}"
...
output_dir="${APOLYSIS_F5_CHAOS_PERFORMANCE_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/f5-chaos-performance.XXXXXX")}"
mkdir -p "$output_dir"
...
chaos_delete_count="${APOLYSIS_F5_CHAOS_DELETE_COUNT:-$(((replicas_total + 4) / 5))}"
if [[ "$chaos_delete_count" -lt 1 ]]; then
    chaos_delete_count=1
fi
cpu_request_millicores="${APOLYSIS_F5_CHAOS_CPU_REQUEST_MILLICORES:-5}"
cpu_limit_millicores="${APOLYSIS_F5_CHAOS_CPU_LIMIT_MILLICORES:-20}"
memory_request_mib="${APOLYSIS_F5_CHAOS_MEMORY_REQUEST_MIB:-8}"
memory_limit_mib="${APOLYSIS_F5_CHAOS_MEMORY_LIMIT_MIB:-32}"
...
manifest="$output_dir/apolysis-f5-chaos-performance.yaml"
pods_before="$output_dir/apolysis-f5-chaos-pods-before.json"
pods_after="$output_dir/apolysis-f5-chaos-pods-after.json"
deployments_before="$output_dir/apolysis-f5-chaos-deployments-before.json"
deployments_after="$output_dir/apolysis-f5-chaos-deployments-after.json"
events_before="$output_dir/apolysis-f5-chaos-events-before.json"
events_after="$output_dir/apolysis-f5-chaos-events-after.json"
metrics_before="$output_dir/apolysis-f5-chaos-metrics-before.txt"
metrics_after="$output_dir/apolysis-f5-chaos-metrics-after.txt"
deleted_pods_path="$output_dir/apolysis-f5-chaos-deleted-pods.txt"
observations="$output_dir/apolysis-f5-chaos-performance-observations.json"
evidence="$output_dir/apolysis-f5-chaos-performance-evidence.json"
report="$output_dir/apolysis-f5-chaos-performance-report.json"
fail_evidence="$output_dir/apolysis-f5-chaos-performance-evidence-fail.json"
fail_report="$output_dir/apolysis-f5-chaos-performance-report-fail.json"
namespace_deleted=0
...
if [[ "$deployment_count" -lt 3 || "$replicas_total" -lt 30 ]]; then
    echo "apolysis-f5: chaos/performance gate requires at least 3 deployments and 30 replicas" >&2
    exit 1
fi

if ! kubectl top nodes >/dev/null 2>&1; then
    echo "apolysis-f5: metrics-server is required for F5.20 resource evidence" >&2
    exit 1
fi
...
metadata:
  name: ${namespace}
  labels:
    app.kubernetes.io/name: apolysis-f5-chaos-performance
...
kind: Deployment
metadata:
  name: apolysis-f5-chaos-workload-${index}
  namespace: ${namespace}
  labels:
    app.kubernetes.io/name: apolysis-f5-chaos-workload
    app.kubernetes.io/part-of: apolysis
    apolysis.dev/phase: f5.20-chaos-performance
spec:
  ...
  template:
    metadata:
      labels:
        ...
        apolysis.dev/workload-index: "${index}"
      annotations:
        apolysis.dev/session-id: apolysis-f5-chaos-${stamp}-${index}
    spec:
...
for index in $(seq 1 "$deployment_count"); do
    kubectl -n "$namespace" rollout status "deployment/apolysis-f5-chaos-workload-${index}" --timeout=180s
done
kubectl -n "$namespace" wait \
    --for=condition=Ready \
    pod \
    -l app.kubernetes.io/name=apolysis-f5-chaos-workload \
    --timeout=180s
...
kubectl -n "$namespace" get pods -l app.kubernetes.io/name=apolysis-f5-chaos-workload -o json >"$pods_before"
kubectl -n "$namespace" get deployments -l app.kubernetes.io/name=apolysis-f5-chaos-workload -o json >"$deployments_before"
...
PY
    ...
    echo "apolysis-f5: metrics-server did not return pod metrics for all workload pods" >&2
...
kubectl -n "$namespace" get pods -l app.kubernetes.io/name=apolysis-f5-chaos-workload -o json >"$pods_after"
kubectl -n "$namespace" get deployments -l app.kubernetes.io/name=apolysis-f5-chaos-workload -o json >"$deployments_after"
...
if cargo run -q -p apolysis-validation --bin apolysis-f5-chaos-performance-evidence -- \
    --evidence "$fail_evidence" >"$fail_report"; then
    echo "apolysis-f5: invalid chaos/performance evidence unexpectedly passed" >&2
    exit 1
fi
...
printf 'apolysis-f5: chaos/performance live gate passed (%s)\n' "$output_dir"
