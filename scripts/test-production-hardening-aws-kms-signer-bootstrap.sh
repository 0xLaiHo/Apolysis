...
confirm="${APOLYSIS_CONFIRM_PRODUCTION_HARDENING_AWS_KMS_SIGNER_BOOTSTRAP:-0}"
require_ready="${APOLYSIS_REQUIRE_PRODUCTION_HARDENING_AWS_KMS_SIGNER_BOOTSTRAP:-0}"

if [[ "$confirm" != "1" ]]; then
    cat >&2 <<'EOF'
apolysis-production-hardening: AWS KMS signer bootstrap is opt-in.
Set APOLYSIS_CONFIRM_PRODUCTION_HARDENING_AWS_KMS_SIGNER_BOOTSTRAP=1 after confirming the AWS
...
output_dir="${APOLYSIS_PRODUCTION_HARDENING_AWS_KMS_SIGNER_BOOTSTRAP_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/production-hardening-aws-kms-signer-bootstrap.XXXXXX")}"
mkdir -p "$output_dir"
output_dir="$(cd "$output_dir" && pwd)"

report="$output_dir/apolysis-production-hardening-aws-kms-signer-bootstrap-report.json"
...
mode="${APOLYSIS_PRODUCTION_HARDENING_AWS_KMS_BOOTSTRAP_MODE:-inspect}"
key_id="${APOLYSIS_PRODUCTION_HARDENING_AWS_KMS_KEY_ID:-}"
alias_name="${APOLYSIS_PRODUCTION_HARDENING_AWS_KMS_ALIAS:-}"
...
key_spec="${APOLYSIS_PRODUCTION_HARDENING_AWS_KMS_KEY_SPEC:-RSA_2048}"
create_confirm="${APOLYSIS_CONFIRM_PRODUCTION_HARDENING_AWS_KMS_KEY_CREATION:-0}"
...
if key_arn and region:
    next_command = (
        ...
        f"APOLYSIS_PRODUCTION_HARDENING_AWS_KMS_KEY_ID={key_arn} "
        f"APOLYSIS_PRODUCTION_HARDENING_AWS_REGION={region} "
...
report = {
    "schema_version": 1,
    "phase": "production-hardening.aws-kms-signer-bootstrap",
    ...
    "notes": [
        ...
        "production-hardening.aws-kms-signer-bootstrap qualifies or prepares an AWS KMS signer but does not produce retained final signing evidence.",
        "Run scripts/test-production-hardening-aws-kms-signing.sh against the reported key URI to produce the required ProductionHardening signing evidence.",
        "Key creation requires APOLYSIS_PRODUCTION_HARDENING_AWS_KMS_BOOTSTRAP_MODE=ensure and APOLYSIS_CONFIRM_PRODUCTION_HARDENING_AWS_KMS_KEY_CREATION=1.",
    ],
...
fail_with_report() {
    ...
    echo "apolysis-production-hardening: AWS KMS signer bootstrap did not pass ($report)" >&2
    if [[ "$#" -gt 0 ]]; then
...
case "$mode" in
    inspect|ensure) ;;
    *)
        fail_with_report "validate_mode" "APOLYSIS_PRODUCTION_HARDENING_AWS_KMS_BOOTSTRAP_MODE must be inspect or ensure"
        ;;
esac
...
if [[ -z "$aws_region" ]]; then
    fail_with_report "validate_region" "APOLYSIS_PRODUCTION_HARDENING_AWS_REGION_or_AWS_REGION"
fi

if [[ -n "$alias_name" && "$alias_name" != alias/* ]]; then
    fail_with_report "validate_alias" "APOLYSIS_PRODUCTION_HARDENING_AWS_KMS_ALIAS must start with alias/"
fi

if [[ "$mode" == "inspect" && -z "$key_id" && -z "$alias_name" ]]; then
    fail_with_report "validate_key_ref" "APOLYSIS_PRODUCTION_HARDENING_AWS_KMS_KEY_ID_or_APOLYSIS_PRODUCTION_HARDENING_AWS_KMS_ALIAS"
fi

if [[ "$mode" == "ensure" && -z "$alias_name" ]]; then
    fail_with_report "validate_ensure_alias" "APOLYSIS_PRODUCTION_HARDENING_AWS_KMS_ALIAS"
fi
...
if [[ "$mode" == "ensure" ]]; then
    if aws --region "$aws_region" kms describe-key --key-id "$alias_name" --output json >"$key_metadata" 2>"$aws_error_log"; then
        ...
        if [[ "$create_confirm" != "1" ]]; then
            fail_with_report "confirm_key_creation" "APOLYSIS_CONFIRM_PRODUCTION_HARDENING_AWS_KMS_KEY_CREATION"
        fi
        description="${APOLYSIS_PRODUCTION_HARDENING_AWS_KMS_DESCRIPTION:-Apolysis ProductionHardening release signing key}"
        if ! aws --region "$aws_region" kms create-key \
            --key-usage SIGN_VERIFY \
            --key-spec "$key_spec" \
            --origin AWS_KMS \
            --description "$description" \
            --tags TagKey=Project,TagValue=Apolysis TagKey=Phase,TagValue=ProductionHardening TagKey=Purpose,TagValue=release-signing \
...
printf 'apolysis-production-hardening: AWS KMS signer bootstrap gate passed (%s)\n' "$output_dir"
