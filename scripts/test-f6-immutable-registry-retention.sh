...
output_dir="${APOLYSIS_F6_IMMUTABLE_REGISTRY_RETENTION_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/f6-immutable-registry-retention.XXXXXX")}"
mkdir -p "$output_dir"
output_dir="$(cd "$output_dir" && pwd)"

report="$output_dir/apolysis-f6-immutable-registry-retention-report.json"
require_ready="${APOLYSIS_REQUIRE_F6_IMMUTABLE_REGISTRY_RETENTION:-0}"
...
manifest_path = output_dir / "apolysis-f6-immutable-registry-retention-manifest.json"
...
evidence_value = env_value("APOLYSIS_F6_IMMUTABLE_REGISTRY_EVIDENCE")
...
provider = env_value("APOLYSIS_F6_IMMUTABLE_REGISTRY_PROVIDER") or first_text_value(
    evidence_doc, "provider", "registry_provider"
)
registry_uri = env_value("APOLYSIS_F6_IMMUTABLE_REGISTRY_URI") or first_text_value(
    evidence_doc, "registry_uri", "repository_uri", "registry"
)
image_ref = env_value("APOLYSIS_F6_IMMUTABLE_REGISTRY_IMAGE_REF") or first_text_value(
    evidence_doc, "image_ref", "image", "artifact_ref"
)
image_digest = normalize_digest(
    env_value("APOLYSIS_F6_IMMUTABLE_REGISTRY_IMAGE_DIGEST")
    or first_text_value(evidence_doc, "image_digest", "digest", "manifest_digest")
)
policy_id = env_value("APOLYSIS_F6_IMMUTABLE_REGISTRY_POLICY_ID") or first_text_value(
    evidence_doc, "policy_id", "retention_policy_id", "immutability_policy_id"
)
immutability_mode = env_value("APOLYSIS_F6_IMMUTABLE_REGISTRY_MODE") or first_text_value(
    evidence_doc, "immutability_mode", "retention_mode", "policy_mode"
)
control_plane = env_value("APOLYSIS_F6_IMMUTABLE_REGISTRY_CONTROL_PLANE") or first_text_value(
    evidence_doc, "provider_control_plane", "registry_control_plane", "control_plane"
)
retention_until = env_value("APOLYSIS_F6_IMMUTABLE_REGISTRY_RETENTION_UNTIL") or first_text_value(
...
enabled_value = env_value("APOLYSIS_F6_IMMUTABLE_REGISTRY_ENABLED")
if enabled_value:
...
manifest = {
    "schema_version": 1,
    "phase": "F6.9",
    "source": "f6_immutable_registry_retention",
    "provider": provider,
...
report = {
    ...
    "notes": [
        ...
        "The F6.9 gate validates immutable or retention-protected registry metadata for a digest-pinned release image.",
    ...
    "next_commands": {
        "audit": "./scripts/test-f6-immutable-registry-retention.sh",
        "required_metadata": "APOLYSIS_F6_IMMUTABLE_REGISTRY_EVIDENCE=<immutable-registry.json> APOLYSIS_REQUIRE_F6_IMMUTABLE_REGISTRY_RETENTION=1 ./scripts/test-f6-immutable-registry-retention.sh",
    },
...
if require_ready and not immutable_registry_ready:
    print(f"apolysis-f6: immutable registry retention failed closed ({report_path})", file=sys.stderr)
    print("missing requirements: " + ", ".join(report["missing_requirements"]), file=sys.stderr)
    raise SystemExit(1)
PY

cat <<EOF
apolysis-f6: immutable registry retention audit written ($output_dir)
APOLYSIS_F6_IMMUTABLE_REGISTRY_RETENTION_REPORT=$report
EOF
