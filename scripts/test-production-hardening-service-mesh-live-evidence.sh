...
output_dir="${APOLYSIS_PRODUCTION_HARDENING_SERVICE_MESH_LIVE_EVIDENCE_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/production-hardening-service-mesh-live-evidence.XXXXXX")}"
mkdir -p "$output_dir"
...
pass_evidence="$output_dir/apolysis-production-hardening-istio-live-evidence.json"
fail_evidence="$output_dir/apolysis-production-hardening-istio-live-evidence-fail.json"
pass_report="$output_dir/apolysis-production-hardening-istio-live-evidence-report.json"
fail_report="$output_dir/apolysis-production-hardening-istio-live-evidence-fail-report.json"

cat >"$pass_evidence" <<'JSON'
{
  "evidence_id": "production-hardening-istio-mtls-handshake-20260624",
  "source": "live_cluster",
  "provider": "istio",
...
if cargo run -q -p apolysis-validation --bin apolysis-production-hardening-service-mesh-live-evidence -- \
    --evidence "$fail_evidence" >"$fail_report"; then
    echo "apolysis-production-hardening: invalid service-mesh live evidence unexpectedly passed" >&2
    exit 1
fi
...
jq -e '
  ...
  and (.failures | map(.message) | index("Istio is required for this ProductionHardening service-mesh live gate"))
...
printf 'apolysis-production-hardening: service-mesh live evidence gate passed (%s)\n' "$output_dir"
