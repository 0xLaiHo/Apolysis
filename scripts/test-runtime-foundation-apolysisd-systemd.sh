...
unit="apolysisd-runtime-foundation-recovery-$$-$(date +%s%N).service"
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/apolysis-runtime-foundation-apolysisd-systemd.XXXXXX")"
...
wait_for_socket() {
    ...
    echo "apolysis-runtime_foundation: apolysisd socket did not become ready" >&2
    journalctl -u "$unit" -n 80 --no-pager >&2 || true
    return 1
}
...
wait_for_restarted_pid() {
    ...
    echo "apolysis-runtime_foundation: apolysisd did not restart away from PID $previous_pid" >&2
...
echo "apolysis-runtime_foundation: apolysisd systemd restart validation passed"
