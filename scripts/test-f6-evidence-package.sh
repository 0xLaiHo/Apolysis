...
output_dir="${APOLYSIS_F6_EVIDENCE_PACKAGE_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/f6-evidence-package.XXXXXX")}"
mkdir -p "$output_dir"
output_dir="$(cd "$output_dir" && pwd)"

report="$output_dir/apolysis-f6-evidence-package-report.json"
require_ready="${APOLYSIS_REQUIRE_F6_EVIDENCE_PACKAGE:-0}"
...
f5_builder = repo_root / "scripts/build-f5-final-external-provider-bundle.sh"
downstream_dir = output_dir / "f5-final-external-provider-bundle"
downstream_report_path = downstream_dir / "apolysis-f5-final-external-provider-bundle-report.json"
downstream_bundle_path = downstream_dir / "bundle-root/apolysis-f5-final-external-provider-bundle.json"
package_manifest_path = output_dir / "apolysis-f6-evidence-package-manifest.json"
archive_path = output_dir / "apolysis-f6-evidence-package.tar.gz"
archive_sha_path = output_dir / "apolysis-f6-evidence-package.tar.gz.sha256"
...
signing_evidence_value = env_value("APOLYSIS_F6_SIGNING_EVIDENCE", "APOLYSIS_F5_SIGNING_EVIDENCE")
signing_report_value = env_value("APOLYSIS_F6_SIGNING_REPORT", "APOLYSIS_F5_SIGNING_REPORT")
...
worm_evidence = Path(env_value("APOLYSIS_F6_WORM_EVIDENCE", "APOLYSIS_F5_WORM_EVIDENCE") or artifact_from_root("worm-evidence.json"))
worm_report = Path(env_value("APOLYSIS_F6_WORM_REPORT", "APOLYSIS_F5_WORM_REPORT") or artifact_from_root("worm-report.json"))
registry_evidence = Path(env_value("APOLYSIS_F6_REGISTRY_EVIDENCE", "APOLYSIS_F5_REGISTRY_EVIDENCE") or artifact_from_root("registry-evidence.json"))
registry_report = Path(env_value("APOLYSIS_F6_REGISTRY_REPORT", "APOLYSIS_F5_REGISTRY_REPORT") or artifact_from_root("registry-report.json"))
managed_mesh_evidence = Path(
    env_value("APOLYSIS_F6_MANAGED_MESH_EVIDENCE", "APOLYSIS_F5_MANAGED_MESH_EVIDENCE")
    or artifact_from_root("managed-mesh-evidence.json")
)
managed_mesh_report = Path(
    env_value("APOLYSIS_F6_MANAGED_MESH_REPORT", "APOLYSIS_F5_MANAGED_MESH_REPORT")
...
if provider_root is None:
    missing_requirements.append("APOLYSIS_F6_PROVIDER_ARTIFACT_ROOT")
elif not provider_root.is_dir():
    missing_requirements.append("APOLYSIS_F6_PROVIDER_ARTIFACT_ROOT_exists")
...
downstream_output_path = output_dir / "f5-final-external-provider-bundle.out"
...
if not missing_requirements:
    ...
    env.update(
        {
            "APOLYSIS_F5_FINAL_EXTERNAL_BUNDLE_OUTPUT_DIR": str(downstream_dir),
            "APOLYSIS_F5_SIGNING_PROVIDER": signing_provider,
            "APOLYSIS_F5_SIGNING_CONTROL_PLANE": signing_control_plane,
            "APOLYSIS_F5_SIGNING_EVIDENCE": str(signing_evidence),
            "APOLYSIS_F5_SIGNING_REPORT": str(signing_report),
            "APOLYSIS_F5_WORM_PROVIDER": worm_provider,
            "APOLYSIS_F5_WORM_CONTROL_PLANE": worm_control_plane,
            "APOLYSIS_F5_WORM_EVIDENCE": str(worm_evidence),
            "APOLYSIS_F5_WORM_REPORT": str(worm_report),
            "APOLYSIS_F5_REGISTRY_PROVIDER": registry_provider,
            "APOLYSIS_F5_REGISTRY_CONTROL_PLANE": registry_control_plane,
            "APOLYSIS_F5_REGISTRY_EVIDENCE": str(registry_evidence),
            "APOLYSIS_F5_REGISTRY_REPORT": str(registry_report),
            "APOLYSIS_F5_MANAGED_MESH_PROVIDER": managed_mesh_provider,
            "APOLYSIS_F5_MANAGED_MESH_CONTROL_PLANE": managed_mesh_control_plane,
            "APOLYSIS_F5_MANAGED_MESH_EVIDENCE": str(managed_mesh_evidence),
            "APOLYSIS_F5_MANAGED_MESH_REPORT": str(managed_mesh_report),
        }
    )
    ...
    if downstream_exit_code != 0:
        missing_requirements.append("f5_final_external_provider_bundle_succeeded")
    if downstream_doc.get("passed") is not True:
        missing_requirements.append("f5_final_external_provider_bundle_passed")
    ...
    if downstream_bundle_path.is_file():
        ...
        missing_requirements.append("f5_final_external_provider_bundle_manifest")
    ...
    if bundle_root.is_dir():
        ...
        archive_sha = ""
        missing_requirements.append("f6_evidence_package_bundle_root")
else:
    downstream_exit_code = 0
    archive_sha = ""
...
package_manifest = {
    "schema_version": 1,
    "phase": "F6.6",
    "package_id": f"f6-evidence-package-{generated_at_unix_ms}",
    "source": "f6_evidence_package",
...
report = {
    ...
    "notes": [
        ...
        "The F6.6 package gate wraps the historical F5 final external-provider bundle builder.",
...
if require_ready and not evidence_package_ready:
    print(f"apolysis-f6: evidence package failed closed ({report_path})", file=sys.stderr)
    print("missing requirements: " + ", ".join(report["missing_requirements"]), file=sys.stderr)
    raise SystemExit(1)
PY

cat <<EOF
apolysis-f6: evidence package audit written ($output_dir)
APOLYSIS_F6_EVIDENCE_PACKAGE_REPORT=$report
EOF
