...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/apolysis-runtime-foundation.XXXXXX")"
...
start_daemon() {
    ...
    echo "apolysis-runtime_foundation: daemon socket did not become ready" >&2
    cat "$LOG" >&2
    return 1
}
...
echo "apolysis-runtime_foundation: foundation validation passed"
