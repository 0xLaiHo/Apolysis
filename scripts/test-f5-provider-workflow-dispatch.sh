...
output_dir="${APOLYSIS_F5_PROVIDER_WORKFLOW_DISPATCH_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/f5-provider-workflow-dispatch.XXXXXX")}"
mkdir -p "$output_dir"
output_dir="$(cd "$output_dir" && pwd)"

report="$output_dir/apolysis-f5-provider-workflow-dispatch-report.json"
dispatch_command_file="$output_dir/gh-workflow-dispatch-command.sh"
require_ready="${APOLYSIS_REQUIRE_F5_PROVIDER_WORKFLOW_DISPATCH:-0}"
...
mode = os.environ.get("APOLYSIS_F5_PROVIDER_WORKFLOW_DISPATCH_MODE", "dry-run")
dispatch_confirmed = os.environ.get("APOLYSIS_CONFIRM_F5_PROVIDER_WORKFLOW_DISPATCH", "0") == "1"
repo = origin_repo()
github_ref = os.environ.get("APOLYSIS_F5_GITHUB_REF", "f5-production-hardening")

run_aws_kms = bool_env("APOLYSIS_F5_DISPATCH_RUN_AWS_KMS", True)
run_aws_kms_bootstrap = bool_env("APOLYSIS_F5_DISPATCH_RUN_AWS_KMS_BOOTSTRAP", True)
aws_kms_bootstrap_mode = os.environ.get("APOLYSIS_F5_DISPATCH_AWS_KMS_BOOTSTRAP_MODE", "inspect")
confirm_aws_kms_key_creation = bool_env("APOLYSIS_F5_DISPATCH_CONFIRM_AWS_KMS_KEY_CREATION", False)
run_gke_mesh = bool_env("APOLYSIS_F5_DISPATCH_RUN_GKE_MESH", False)
retained_signing_provider_artifact = bool_env("APOLYSIS_F5_DISPATCH_RETAINED_SIGNING_PROVIDER_ARTIFACT", False)
retained_managed_mesh_provider_artifact = bool_env("APOLYSIS_F5_DISPATCH_RETAINED_MANAGED_MESH_PROVIDER_ARTIFACT", True)
assemble_final_bundle = bool_env("APOLYSIS_F5_DISPATCH_ASSEMBLE_FINAL_BUNDLE", True)
retained_provider_artifact_run_id = os.environ.get("APOLYSIS_F5_RETAINED_PROVIDER_ARTIFACT_RUN_ID", "")
retained_provider_artifact_url = os.environ.get("APOLYSIS_F5_RETAINED_PROVIDER_ARTIFACT_URL", "")
retained_provider_artifact_sha256 = os.environ.get("APOLYSIS_F5_RETAINED_PROVIDER_ARTIFACT_SHA256", "")
...
if mode not in {"dry-run", "dispatch"}:
    missing_requirements.append("APOLYSIS_F5_PROVIDER_WORKFLOW_DISPATCH_MODE_dry-run_or_dispatch")
...
if mode == "dispatch":
    if not dispatch_confirmed:
        missing_requirements.append("APOLYSIS_CONFIRM_F5_PROVIDER_WORKFLOW_DISPATCH")
...
command = [
    ...
    "workflow",
    "run",
    "f5-final-provider-evidence.yml",
    "--repo",
    repo or "<owner>/<repo>",
    "--ref",
    github_ref or "f5-production-hardening",
    "-f",
...
dispatch_plan_ready = not missing_requirements or (
    mode == "dispatch" and missing_requirements == ["APOLYSIS_CONFIRM_F5_PROVIDER_WORKFLOW_DISPATCH"]
)
...
report = {
    ...
    "notes": [
        ...
        "Set APOLYSIS_F5_PROVIDER_WORKFLOW_DISPATCH_MODE=dispatch and APOLYSIS_CONFIRM_F5_PROVIDER_WORKFLOW_DISPATCH=1 before gh workflow run is executed.",
        ...
        "This gate does not create signing evidence; the dispatched workflow must still run F5.25 or consume retained external HSM evidence.",
    ],
    "next_commands": {
        ...
        "dispatch_dry_run_required": "APOLYSIS_REQUIRE_F5_PROVIDER_WORKFLOW_DISPATCH=1 ./scripts/test-f5-provider-workflow-dispatch.sh",
        "dispatch_confirmed": "APOLYSIS_F5_PROVIDER_WORKFLOW_DISPATCH_MODE=dispatch APOLYSIS_CONFIRM_F5_PROVIDER_WORKFLOW_DISPATCH=1 APOLYSIS_REQUIRE_F5_PROVIDER_WORKFLOW_DISPATCH=1 ./scripts/test-f5-provider-workflow-dispatch.sh",
    },
...
if require_ready and not ready:
    print(f"apolysis-f5: provider workflow dispatch failed closed ({report_path})", file=sys.stderr)
    print("missing requirements: " + ", ".join(missing_requirements), file=sys.stderr)
    raise SystemExit(1)
PY

cat <<EOF
apolysis-f5: provider workflow dispatch audit written ($output_dir)
APOLYSIS_F5_PROVIDER_WORKFLOW_DISPATCH_REPORT=$report
EOF
