...
output_dir="${APOLYSIS_PRODUCTION_HARDENING_FINAL_PROVIDER_COMPLETION_TEST_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/production-hardening-final-provider-completion-test.XXXXXX")}"
mkdir -p "$output_dir"
...
completion_gate="$repo_root/scripts/verify-production-hardening-final-provider-completion.sh"
if [[ ! -s "$completion_gate" ]]; then
    echo "apolysis-production-hardening: missing production-hardening.final-provider-completion final provider completion verifier: $completion_gate" >&2
    exit 1
fi
...
write_evidence "$signing_evidence" "cloud_kms" "key_uri" "awskms://arn:aws:kms:us-west-2:111122223333:key/apolysis-production-hardening"
write_report "$signing_report" "cloud_kms"
write_evidence "$worm_evidence" "cloudflare_r2_bucket_lock" "bucket_uri" "cloudflare-r2:e85b6fa3634dc882cfbd2188361fb37e:apolysis-production-hardening-worm"
write_report "$worm_report" "cloudflare_r2_bucket_lock"
write_evidence "$registry_evidence" "docker_hub" "registry_uri" "docker-hub:devlaiho:apolysis-production-hardening-registry"
...
APOLYSIS_PRODUCTION_HARDENING_FINAL_PROVIDER_COMPLETION_OUTPUT_DIR="$success_dir" \
...
completion_report="$success_dir/apolysis-production-hardening-final-provider-completion-report.json"
jq -e '
  .passed == true
  ...
  and (.final_bundle_report | test("apolysis-production-hardening-final-external-provider-bundle-report.json$"))
' "$completion_report" >/dev/null

set +e
APOLYSIS_PRODUCTION_HARDENING_FINAL_PROVIDER_COMPLETION_OUTPUT_DIR="$failure_dir" \
APOLYSIS_PRODUCTION_HARDENING_SIGNING_EVIDENCE="$signing_evidence" \
APOLYSIS_PRODUCTION_HARDENING_SIGNING_REPORT="$signing_report" \
APOLYSIS_PRODUCTION_HARDENING_WORM_EVIDENCE="$worm_evidence" \
APOLYSIS_PRODUCTION_HARDENING_WORM_REPORT="$worm_report" \
...
if [[ "$completion_rc" -eq 0 ]]; then
    echo "apolysis-production-hardening: final provider completion unexpectedly passed without managed mesh evidence" >&2
    exit 1
fi
...
' "$failure_dir/apolysis-production-hardening-final-provider-completion-report.json" >/dev/null

printf 'apolysis-production-hardening: final provider completion gate contract passed (%s)\n' "$output_dir"
