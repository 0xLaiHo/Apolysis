...
output_dir="${APOLYSIS_REGULATED_RELEASE_EXTERNAL_RETENTION_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/regulated-release-external-retention.XXXXXX")}"
mkdir -p "$output_dir"
output_dir="$(cd "$output_dir" && pwd)"

report="$output_dir/apolysis-regulated-release-external-retention-report.json"
require_ready="${APOLYSIS_REQUIRE_REGULATED_RELEASE_EXTERNAL_RETENTION:-0}"
...
manifest_path = output_dir / "apolysis-regulated-release-external-retention-manifest.json"
...
retained_report_value = env_value("APOLYSIS_REGULATED_RELEASE_RETAINED_EVIDENCE_PACKAGE_REPORT")
external_evidence_value = env_value("APOLYSIS_REGULATED_RELEASE_EXTERNAL_RETENTION_EVIDENCE")
...
if retained_report is None:
    missing_requirements.append("APOLYSIS_REGULATED_RELEASE_RETAINED_EVIDENCE_PACKAGE_REPORT")
...
for entry in retained_files:
    ...
    if path_value.endswith("apolysis-regulated-release-evidence-package.tar.gz"):
...
provider = env_value("APOLYSIS_REGULATED_RELEASE_EXTERNAL_RETENTION_PROVIDER") or first_text_value(
    external_doc, "provider", "retention_provider"
)
retention_mode = env_value("APOLYSIS_REGULATED_RELEASE_EXTERNAL_RETENTION_MODE") or first_text_value(
    external_doc, "retention_mode", "object_lock_mode"
)
object_uri = env_value("APOLYSIS_REGULATED_RELEASE_EXTERNAL_RETENTION_URI") or first_text_value(
    external_doc, "object_uri", "retention_uri", "archive_uri"
)
object_version_id = env_value("APOLYSIS_REGULATED_RELEASE_EXTERNAL_RETENTION_VERSION_ID") or first_text_value(
    external_doc, "object_version_id", "version_id", "generation"
)
retention_until = env_value("APOLYSIS_REGULATED_RELEASE_EXTERNAL_RETENTION_UNTIL") or first_text_value(
    external_doc, "retention_until", "retain_until", "retain_until_date"
)
control_plane = env_value("APOLYSIS_REGULATED_RELEASE_EXTERNAL_RETENTION_CONTROL_PLANE") or first_text_value(
    external_doc, "provider_control_plane", "retention_control_plane", "bucket_uri"
)
archive_sha = normalize_sha(
    env_value("APOLYSIS_REGULATED_RELEASE_EXTERNAL_RETENTION_ARCHIVE_SHA256")
...
manifest = {
    "schema_version": 1,
    "phase": "regulated-release.external-retention",
    "source": "regulated_release_external_retention",
    "provider": provider,
...
report = {
    ...
    "notes": [
        ...
        "The regulated-release.external-retention gate validates external WORM/object-lock retention metadata for an already retained RegulatedRelease evidence package.",
    ...
    "next_commands": {
        "audit": "./scripts/test-regulated-release-external-retention.sh",
        "required_metadata": "APOLYSIS_REGULATED_RELEASE_RETAINED_EVIDENCE_PACKAGE_REPORT=<retained-report> APOLYSIS_REGULATED_RELEASE_EXTERNAL_RETENTION_EVIDENCE=<external-retention.json> APOLYSIS_REQUIRE_REGULATED_RELEASE_EXTERNAL_RETENTION=1 ./scripts/test-regulated-release-external-retention.sh",
    },
...
if require_ready and not external_retention_ready:
    print(f"apolysis-regulated_release: external retention failed closed ({report_path})", file=sys.stderr)
    print("missing requirements: " + ", ".join(report["missing_requirements"]), file=sys.stderr)
    raise SystemExit(1)
PY

cat <<EOF
apolysis-regulated_release: external retention audit written ($output_dir)
APOLYSIS_REGULATED_RELEASE_EXTERNAL_RETENTION_REPORT=$report
EOF
