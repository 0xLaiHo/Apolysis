...
repo = origin_repo()
mode = os.environ.get("APOLYSIS_F5_AWS_OIDC_HANDOFF_MODE", "audit")
apply_confirmed = os.environ.get("APOLYSIS_CONFIRM_F5_AWS_OIDC_APPLY", "0") == "1"
...
oidc_thumbprint = os.environ.get("APOLYSIS_F5_GITHUB_OIDC_THUMBPRINT", "")
...
if mode not in {"audit", "inspect", "ensure"}:
    missing_requirements.append("APOLYSIS_F5_AWS_OIDC_HANDOFF_MODE_audit_inspect_or_ensure")
...
if mode in {"inspect", "ensure"}:
    if not apply_confirmed:
        missing_requirements.append("APOLYSIS_CONFIRM_F5_AWS_OIDC_APPLY")
...
if mode in {"inspect", "ensure"} and base_ready and apply_confirmed and tools["aws"]["available"]:
    ...
    if rc == 0:
        ...
        if not oidc_thumbprint:
            missing_requirements.append("APOLYSIS_F5_GITHUB_OIDC_THUMBPRINT")
...
next_commands = {
    ...
    "inspect_aws_iam": "APOLYSIS_F5_AWS_OIDC_HANDOFF_MODE=inspect APOLYSIS_CONFIRM_F5_AWS_OIDC_APPLY=1 APOLYSIS_REQUIRE_F5_AWS_OIDC_HANDOFF=1 ./scripts/test-f5-aws-oidc-handoff.sh",
    "ensure_aws_iam": "APOLYSIS_F5_AWS_OIDC_HANDOFF_MODE=ensure APOLYSIS_CONFIRM_F5_AWS_OIDC_APPLY=1 APOLYSIS_F5_GITHUB_OIDC_THUMBPRINT=<github-actions-oidc-thumbprint> APOLYSIS_REQUIRE_F5_AWS_OIDC_HANDOFF=1 ./scripts/test-f5-aws-oidc-handoff.sh",
