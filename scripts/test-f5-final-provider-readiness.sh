...
output_dir="${APOLYSIS_F5_FINAL_PROVIDER_READINESS_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/f5-final-provider-readiness.XXXXXX")}"
mkdir -p "$output_dir"
output_dir="$(cd "$output_dir" && pwd)"

report="$output_dir/apolysis-f5-final-provider-readiness-report.json"
...
require_ready = os.environ.get("APOLYSIS_REQUIRE_F5_FINAL_PROVIDER_READINESS", "0") == "1"
...
known_defaults = {
    "APOLYSIS_F5_WORM_EVIDENCE": repo_root / "target/f5-cloudflare-r2-worm/apolysis-f5-cloudflare-r2-worm-evidence.json",
    "APOLYSIS_F5_WORM_REPORT": repo_root / "target/f5-cloudflare-r2-worm/apolysis-f5-cloudflare-r2-worm-report.json",
    "APOLYSIS_F5_REGISTRY_EVIDENCE": repo_root / "target/f5-dockerhub-registry-promotion.aByXvA/apolysis-f5-dockerhub-registry-promotion-evidence.json",
    "APOLYSIS_F5_REGISTRY_REPORT": repo_root / "target/f5-dockerhub-registry-promotion.aByXvA/apolysis-f5-dockerhub-registry-promotion-report.json",
}

artifact_envs = [
    "APOLYSIS_F5_SIGNING_EVIDENCE",
    "APOLYSIS_F5_SIGNING_REPORT",
    "APOLYSIS_F5_WORM_EVIDENCE",
    "APOLYSIS_F5_WORM_REPORT",
    "APOLYSIS_F5_REGISTRY_EVIDENCE",
    "APOLYSIS_F5_REGISTRY_REPORT",
    "APOLYSIS_F5_MANAGED_MESH_EVIDENCE",
...
live_prerequisites = {
    ...
    "gke_anthos_service_mesh": {
        ...
        "APOLYSIS_F5_GKE_MESH_FLEET_PROJECT": bool(os.environ.get("APOLYSIS_F5_GKE_MESH_FLEET_PROJECT")),
        "APOLYSIS_F5_GKE_MESH_MEMBERSHIP": bool(os.environ.get("APOLYSIS_F5_GKE_MESH_MEMBERSHIP")),
...
report = {
    ...
    "notes": [
        ...
        "Default mode is an audit. Set APOLYSIS_REQUIRE_F5_FINAL_PROVIDER_READINESS=1 to fail closed until all final provider evidence is present.",
        "F5 completion still requires real provider signing evidence, real managed service-mesh provider evidence, and a passing final external provider bundle.",
    ],
...
if require_ready and not final_provider_ready:
    print(f"apolysis-f5: final provider readiness failed closed ({report_path})", file=sys.stderr)
    print("missing requirements: " + ", ".join(missing_requirements), file=sys.stderr)
    raise SystemExit(1)
PY

cat <<EOF
apolysis-f5: final provider readiness audit written ($output_dir)
APOLYSIS_F5_FINAL_PROVIDER_READINESS_REPORT=$report
EOF
