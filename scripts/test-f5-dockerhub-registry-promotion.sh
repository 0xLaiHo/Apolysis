...
if [[ "${APOLYSIS_F5_DOCKER_HUB_LIVE:-}" != "1" ]]; then
    cat >&2 <<'EOF'
apolysis-f5: Docker Hub live registry promotion is opt-in.
Set APOLYSIS_F5_DOCKER_HUB_LIVE=1 after confirming the Docker Hub account,
...
output_dir="${APOLYSIS_F5_DOCKER_HUB_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/f5-dockerhub-registry-promotion.XXXXXX")}"
mkdir -p "$output_dir"
output_dir="$(cd "$output_dir" && pwd)"

docker_config="${DOCKER_CONFIG:-$HOME/.docker}/config.json"
repository="${APOLYSIS_F5_DOCKER_HUB_REPOSITORY:-apolysis-f5-registry}"
repository_private="${APOLYSIS_F5_DOCKER_HUB_PRIVATE:-true}"
description="Apolysis F5 live Docker Hub registry promotion evidence"
source_tag="${APOLYSIS_F5_DOCKER_HUB_SOURCE_TAG:-staging-f5-$stamp}"
target_tag="${APOLYSIS_F5_DOCKER_HUB_TARGET_TAG:-prod-f5-$stamp}"
rollback_tag="${APOLYSIS_F5_DOCKER_HUB_ROLLBACK_TAG:-rollback-f5-$stamp}"
overwrite_tag="${APOLYSIS_F5_DOCKER_HUB_OVERWRITE_TAG:-overwrite-f5-$stamp}"
immutable_rule="${APOLYSIS_F5_DOCKER_HUB_IMMUTABLE_RULE:-prod-f5-.*}"
retention_days="${APOLYSIS_F5_DOCKER_HUB_RETENTION_DAYS:-180}"

evidence="$output_dir/apolysis-f5-dockerhub-registry-promotion-evidence.json"
report="$output_dir/apolysis-f5-dockerhub-registry-promotion-report.json"
observations="$output_dir/apolysis-f5-dockerhub-registry-promotion-observations.json"
...
if [[ ! -s "$docker_config" ]]; then
    echo "apolysis-f5: Docker config not found at $docker_config" >&2
    exit 1
fi
...
if [[ -z "$docker_auth_entry" ]]; then
    echo "apolysis-f5: Docker Hub login entry is missing from $docker_config" >&2
    exit 1
fi
...
namespace="${APOLYSIS_F5_DOCKER_HUB_NAMESPACE:-$docker_hub_user}"
...
if [[ -z "$docker_hub_user" || -z "$docker_hub_secret" || "$docker_hub_user" == "$docker_hub_secret" ]]; then
    echo "apolysis-f5: Docker Hub credential is malformed" >&2
    exit 1
fi
...
if [[ -z "$hub_token" ]]; then
    echo "apolysis-f5: Docker Hub API login did not return a token" >&2
    exit 1
fi
...
if [[ "$repo_status" == "404" ]]; then
    ...
    if [[ "$create_status" != "201" ]]; then
        echo "apolysis-f5: Docker Hub repository creation failed with HTTP $create_status" >&2
        jq . "$repo_before" >&2 || cat "$repo_before" >&2
        exit 1
    fi
elif [[ "$repo_status" != "200" ]]; then
    echo "apolysis-f5: Docker Hub repository lookup failed with HTTP $repo_status" >&2
    jq . "$repo_before" >&2 || cat "$repo_before" >&2
    exit 1
fi
...
if [[ "$immutable_status" != "200" ]]; then
    echo "apolysis-f5: Docker Hub immutable tag configuration failed with HTTP $immutable_status" >&2
...
if [[ -z "$registry_token" ]]; then
    echo "apolysis-f5: Docker registry auth did not return a token" >&2
    exit 1
fi
...
printf 'apolysis f5 docker hub registry promotion %s\n' "$stamp" >"$image_context/payload.txt"
cat >"$image_context/Dockerfile" <<'DOCKERFILE'
FROM scratch
LABEL org.opencontainers.image.title="apolysis-f5-dockerhub-registry-promotion"
COPY payload.txt /payload.txt
DOCKERFILE
printf 'apolysis f5 docker hub immutable overwrite probe %s\n' "$stamp" >"$overwrite_context/payload.txt"
...
if [[ ! "$image_digest" =~ ^sha256:[0-9a-f]{64}$ ]]; then
    echo "apolysis-f5: Docker Hub did not return a valid staging digest" >&2
    exit 1
fi
if [[ -z "$manifest_media_type" ]]; then
    echo "apolysis-f5: Docker Hub did not return a manifest content type" >&2
    exit 1
fi
...
if [[ "$promotion_status" != "201" && "$promotion_status" != "202" ]]; then
    echo "apolysis-f5: Docker Hub production manifest promotion failed with HTTP $promotion_status" >&2
    exit 1
fi
...
if [[ "$rollback_status" != "201" && "$rollback_status" != "202" ]]; then
    echo "apolysis-f5: Docker Hub rollback manifest promotion failed with HTTP $rollback_status" >&2
    exit 1
fi
...
if [[ "$verify_status" != "200" ]]; then
    echo "apolysis-f5: Docker Hub immutable tag verification failed with HTTP $verify_status" >&2
...
evidence = {
    "evidence_id": f"f5-docker-hub-registry-promotion-{observed_at_unix_ms}",
...
printf 'apolysis-f5: Docker Hub registry promotion gate passed (%s)\n' "$output_dir"
