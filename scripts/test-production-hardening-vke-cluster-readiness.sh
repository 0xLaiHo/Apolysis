...
confirm="${APOLYSIS_CONFIRM_PRODUCTION_HARDENING_VKE_CLUSTER_READINESS:-0}"

if [[ "$confirm" != "1" ]]; then
    cat >&2 <<'EOF'
apolysis-production-hardening: Vultr VKE cluster readiness is opt-in.
Set APOLYSIS_CONFIRM_PRODUCTION_HARDENING_VKE_CLUSTER_READINESS=1 after confirming the kubeconfig
...
expected_nodes="${APOLYSIS_PRODUCTION_HARDENING_VKE_EXPECTED_NODES:-3}"
provider="${APOLYSIS_PRODUCTION_HARDENING_VKE_PROVIDER:-vultr_vke}"
cluster_name="${APOLYSIS_PRODUCTION_HARDENING_VKE_CLUSTER_NAME:-vultr-vke}"
expected_runtime_prefix="${APOLYSIS_PRODUCTION_HARDENING_VKE_EXPECTED_RUNTIME_PREFIX:-containerd://}"

if [[ "$provider" != "vultr_vke" ]]; then
    echo "apolysis-production-hardening: production-hardening.vke-cluster-readiness VKE readiness provider must be vultr_vke" >&2
    exit 2
fi

if ! [[ "$expected_nodes" =~ ^[0-9]+$ ]] || [[ "$expected_nodes" -lt 1 ]]; then
    echo "apolysis-production-hardening: APOLYSIS_PRODUCTION_HARDENING_VKE_EXPECTED_NODES must be a positive integer" >&2
    exit 2
fi

mkdir -p "$repo_root/target"
output_dir="${APOLYSIS_PRODUCTION_HARDENING_VKE_CLUSTER_READINESS_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/production-hardening-vke-cluster-readiness.XXXXXX")}"
mkdir -p "$output_dir"
...
evidence="$output_dir/apolysis-production-hardening-vke-cluster-readiness-evidence.json"
report="$output_dir/apolysis-production-hardening-vke-cluster-readiness-report.json"
...
if ! kubectl top nodes >"$top_nodes" 2>"$top_nodes_error"; then
    echo "apolysis-production-hardening: metrics-server is required for Vultr VKE ProductionHardening readiness evidence" >&2
...
evidence = {
    "schema_version": 1,
    "evidence_id": f"production-hardening-vke-cluster-readiness-{observed_at_unix_ms}",
    "phase": "production-hardening.vke-cluster-readiness",
    "source": "live_cluster",
    "provider": provider,
...
cat <<EOF
apolysis-production-hardening: Vultr VKE cluster readiness passed ($output_dir)
APOLYSIS_PRODUCTION_HARDENING_VKE_CLUSTER_READINESS_EVIDENCE=$evidence
APOLYSIS_PRODUCTION_HARDENING_VKE_CLUSTER_READINESS_REPORT=$report
EOF
