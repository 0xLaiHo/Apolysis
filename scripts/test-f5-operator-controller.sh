...
confirm="${APOLYSIS_CONFIRM_F5_OPERATOR_CONTROLLER:-0}"

if [[ "$confirm" != "1" ]]; then
    cat >&2 <<'EOF'
apolysis-f5: refusing to run live operator/controller validation without confirmation.
Set APOLYSIS_CONFIRM_F5_OPERATOR_CONTROLLER=1 to build/import a local controller
...
output_dir="${APOLYSIS_F5_OPERATOR_CONTROLLER_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/f5-operator-controller.XXXXXX")}"
mkdir -p "$output_dir"
output_dir="$(cd "$output_dir" && pwd)"

namespace="${APOLYSIS_F5_OPERATOR_NAMESPACE:-apolysis-f5-operator-$stamp}"
cluster_name="${APOLYSIS_F5_OPERATOR_CLUSTER_NAME:-mactavish-k3s}"
crd_name="apolysisproductionconfigs.apolysis.dev"
cr_name="${APOLYSIS_F5_OPERATOR_CR_NAME:-platform-production}"
controller_name="apolysis-f5-operator-controller"
leader_lease="apolysis-f5-operator-leader"
managed_daemonset="apolysisd-managed"
managed_configmap="apolysisd-managed-config"
base_image="${APOLYSIS_F5_OPERATOR_BASE_IMAGE:-archlinux:base}"
managed_image="${APOLYSIS_F5_OPERATOR_MANAGED_IMAGE:-alpine:3.20}"
controller_image="localhost/apolysis-f5-operator-controller:$stamp"
image_context="$output_dir/image-context"
image_tar="$output_dir/apolysis-f5-operator-images.tar"
crd_manifest="$output_dir/apolysis-f5-operator-crd.yaml"
controller_manifest="$output_dir/apolysis-f5-operator-controller.yaml"
custom_resource_manifest="$output_dir/apolysis-f5-operator-cr.yaml"
crd_observed="$output_dir/apolysis-f5-operator-crd-observed.json"
cr_observed="$output_dir/apolysis-f5-operator-cr-observed.json"
deployment_observed="$output_dir/apolysis-f5-operator-deployment-observed.json"
lease_observed="$output_dir/apolysis-f5-operator-lease-observed.json"
daemonset_observed="$output_dir/apolysis-f5-operator-daemonset-observed.json"
configmap_observed="$output_dir/apolysis-f5-operator-configmap-observed.json"
observations="$output_dir/apolysis-f5-operator-controller-observations.json"
evidence="$output_dir/apolysis-f5-operator-controller-evidence.json"
report="$output_dir/apolysis-f5-operator-controller-report.json"
fail_evidence="$output_dir/apolysis-f5-operator-controller-evidence-fail.json"
fail_report="$output_dir/apolysis-f5-operator-controller-report-fail.json"
namespace_deleted=0
crd_deleted=0
...
cleanup() {
    ...
    if [[ "$crd_deleted" != "1" ]]; then
        if kubectl get crd "$crd_name" -o jsonpath='{.metadata.labels.apolysis\.dev/f5-gate}' 2>/dev/null | grep -qx 'operator-controller'; then
...
if kubectl get namespace "$namespace" >/dev/null 2>&1; then
    echo "apolysis-f5: namespace already exists: $namespace" >&2
    exit 1
fi

if kubectl get crd "$crd_name" >/dev/null 2>&1; then
    existing_gate="$(kubectl get crd "$crd_name" -o jsonpath='{.metadata.labels.apolysis\.dev/f5-gate}' 2>/dev/null || true)"
    if [[ "$existing_gate" != "operator-controller" ]]; then
        echo "apolysis-f5: CRD $crd_name already exists and is not owned by this gate" >&2
        exit 1
    fi
fi
...
for image in "$base_image" "$managed_image"; do
    if ! docker image inspect "$image" >/dev/null 2>&1; then
        echo "apolysis-f5: missing Docker image: $image" >&2
        echo "apolysis-f5: pull it first or override the APOLYSIS_F5_OPERATOR_*_IMAGE variable" >&2
        exit 1
    fi
done
...
leader_lease="${APOLYSIS_OPERATOR_LEASE:-apolysis-f5-operator-leader}"
interval="${APOLYSIS_OPERATOR_RECONCILE_INTERVAL_SECONDS:-2}"
controller_identity="${HOSTNAME:-apolysis-f5-operator-controller}"
...
metadata:
  ...
  namespace: ${namespace}
  labels:
    app.kubernetes.io/name: apolysis-f5-operator-controller
  ...
  template:
    metadata:
      labels:
        ...
        apolysis.dev/tenant-id: ${tenant_id}
      annotations:
        apolysis.dev/phase: "f5.19-operator-controller"
...
metadata:
  ...
  labels:
    ...
    app.kubernetes.io/part-of: apolysis
    apolysis.dev/f5-gate: operator-controller
spec:
  group: apolysis.dev
  scope: Namespaced
  names:
...
if [[ "$can_cluster_admin" != "no" || "$can_get_secrets" != "no" || "$can_patch_status" != "yes" || "$can_patch_daemonsets" != "yes" ]]; then
    echo "apolysis-f5: controller RBAC preflight failed" >&2
...
if [[ "$delete_cleanup_verified" != "1" ]]; then
    echo "apolysis-f5: managed resources were not garbage-collected after custom resource deletion" >&2
...
evidence = {
    "evidence_id": f"f5-operator-controller-{observed_at_unix_ms}",
    "source": "live_cluster",
...
cargo run -q -p apolysis-validation --bin apolysis-f5-operator-controller-evidence -- \
...
if cargo run -q -p apolysis-validation --bin apolysis-f5-operator-controller-evidence -- \
    --evidence "$fail_evidence" >"$fail_report"; then
    echo "apolysis-f5: invalid operator/controller evidence unexpectedly passed" >&2
    exit 1
fi
...
printf 'apolysis-f5: operator/controller live gate passed (%s)\n' "$output_dir"
