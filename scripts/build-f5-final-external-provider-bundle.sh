...
output_dir="${APOLYSIS_F5_FINAL_EXTERNAL_BUNDLE_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/f5-final-external-provider-bundle.XXXXXX")}"
mkdir -p "$output_dir"
output_dir="$(cd "$output_dir" && pwd)"

bundle_root="$output_dir/bundle-root"
bundle="$bundle_root/apolysis-f5-final-external-provider-bundle.json"
report="$output_dir/apolysis-f5-final-external-provider-bundle-report.json"
...
require_env() {
    ...
    local value="${!name:-}"
    if [[ -z "$value" ]]; then
        echo "apolysis-f5: $name is required for the final external provider bundle" >&2
        exit 2
    fi
    printf '%s' "$value"
}
...
require_file() {
    ...
    local label="$2"
    if [[ ! -f "$path" ]]; then
        echo "apolysis-f5: missing retained $label artifact: $path" >&2
        exit 2
    fi
}
...
signing_evidence="$(require_env APOLYSIS_F5_SIGNING_EVIDENCE)"
signing_report="$(require_env APOLYSIS_F5_SIGNING_REPORT)"
worm_evidence="$(require_env APOLYSIS_F5_WORM_EVIDENCE)"
worm_report="$(require_env APOLYSIS_F5_WORM_REPORT)"
registry_evidence="$(require_env APOLYSIS_F5_REGISTRY_EVIDENCE)"
registry_report="$(require_env APOLYSIS_F5_REGISTRY_REPORT)"
managed_mesh_evidence="$(require_env APOLYSIS_F5_MANAGED_MESH_EVIDENCE)"
managed_mesh_report="$(require_env APOLYSIS_F5_MANAGED_MESH_REPORT)"
...
python3 - "$bundle" \
    ...
    "${APOLYSIS_F5_SIGNING_CONTROL_PLANE:-$(jq -r '.key_uri // .approval.key_uri // empty' "$signing_evidence")}" \
    "$signing_evidence_ref" "$signing_evidence_sha" "$signing_report_ref" "$signing_report_sha" \
    "${APOLYSIS_F5_WORM_PROVIDER:-$(jq -r '.provider // .approval.provider // empty' "$worm_evidence")}" \
    "${APOLYSIS_F5_WORM_CONTROL_PLANE:-$(jq -r '.bucket_uri // .approval.bucket_uri // .endpoint_uri // empty' "$worm_evidence")}" \
    "$worm_evidence_ref" "$worm_evidence_sha" "$worm_report_ref" "$worm_report_sha" \
    "${APOLYSIS_F5_REGISTRY_PROVIDER:-$(jq -r '.provider // .approval.provider // empty' "$registry_evidence")}" \
    "${APOLYSIS_F5_REGISTRY_CONTROL_PLANE:-$(jq -r '.registry_uri // .approval.registry_uri // empty' "$registry_evidence")}" \
    "$registry_evidence_ref" "$registry_evidence_sha" "$registry_report_ref" "$registry_report_sha" \
    "$(require_env APOLYSIS_F5_MANAGED_MESH_PROVIDER)" \
    "$(require_env APOLYSIS_F5_MANAGED_MESH_CONTROL_PLANE)" \
...
bundle = {
    "bundle_id": f"f5-final-external-provider-bundle-{generated_at_unix_ms}",
...
printf 'apolysis-f5: final external provider bundle passed (%s)\n' "$output_dir"
