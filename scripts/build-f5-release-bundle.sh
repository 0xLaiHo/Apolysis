...
payload_tar="$output_dir/apolysis-f5-release-payload.tar.gz"
image_tar="$output_dir/apolysis-f5-apolysisd-image.tar"
...
signing_key="${APOLYSIS_F5_RELEASE_SIGNING_KEY:-}"
signing_pub="${APOLYSIS_F5_RELEASE_SIGNING_PUB:-}"
key_mode="external"
tmp_key_dir=""
remove_image="${APOLYSIS_F5_REMOVE_IMAGE:-1}"
...
if [[ "$(basename "$host_crictl_source")" == "k3s" ]]; then
    ...
    case "$(uname -m)" in
        ...
        *)
            echo "apolysis-f5: unsupported crictl download architecture: $(uname -m)" >&2
            exit 1
            ;;
    esac
...
docker image inspect "$image" >"$output_dir/apolysis-f5-image-inspect.json"
...
if [[ -z "$signing_key" ]]; then
    ...
    COSIGN_PASSWORD="${COSIGN_PASSWORD:-apolysis-f5-local-validation}" \
        cosign generate-key-pair --output-key-prefix "$tmp_key_dir/apolysis-f5-release" >/dev/null
    signing_key="$tmp_key_dir/apolysis-f5-release.key"
    signing_pub="$tmp_key_dir/apolysis-f5-release.pub"
else
    if [[ -z "$signing_pub" ]]; then
        echo "apolysis-f5: APOLYSIS_F5_RELEASE_SIGNING_PUB is required with external signing key" >&2
        exit 1
    fi
fi
...
subjects = [
    ...
    "apolysis-f5-vulnerability-scan.json",
]
...
statement = {
    ...
    "predicate": {
        "buildDefinition": {
            "buildType": "https://apolysis.dev/buildtypes/f5-release-bundle/v1",
            "externalParameters": {
                "phase": "F5.6",
                "image": image,
        ...
        "runDetails": {
            "builder": {"id": "apolysis-local-f5-supply-chain-gate"},
            "metadata": {
                "invocationId": os.environ.get("APOLYSIS_F5_RELEASE_INVOCATION_ID", ""),
...
artifact_names = [
    ...
    "apolysis-f5-image-inspect.json",
]
...
manifest_data = {
    ...
    "image": {
        "tag": image,
        "archive": "apolysis-f5-apolysisd-image.tar",
        "archiveSha256": sha256(output_dir / "apolysis-f5-apolysisd-image.tar"),
    },
    "signing": {
        "keyMode": key_mode,
    ...
    "vulnerabilityPolicy": {
        "scanner": "trivy fs",
        "severity": ["HIGH", "CRITICAL"],
        "maxHighCritical": int(os.environ.get("APOLYSIS_F5_MAX_HIGH_CRITICAL_VULNS", "0")),
    },
}
...
(
    ...
    sha256sum \
        ...
        apolysis-f5-release.pub \
        apolysis-f5-release-manifest.sigstore.json \
        apolysis-f5-provenance.sigstore.json \
        >"$checksums"
)

printf 'apolysis-f5: release supply-chain bundle written to %s\n' "$output_dir"
