...
confirm="${APOLYSIS_CONFIRM_PRODUCTION_HARDENING_SERVICE_MESH_LIVE:-0}"

if [[ "$confirm" != "1" ]]; then
    cat >&2 <<'EOF'
apolysis-production-hardening: refusing to run live Istio service-mesh validation without confirmation.
Set APOLYSIS_CONFIRM_PRODUCTION_HARDENING_SERVICE_MESH_LIVE=1 to install/use Istio in k3s,
...
if [[ -n "${APOLYSIS_PRODUCTION_HARDENING_SERVICE_MESH_LIVE_OUTPUT_DIR:-}" ]]; then
    output_dir="$APOLYSIS_PRODUCTION_HARDENING_SERVICE_MESH_LIVE_OUTPUT_DIR"
    mkdir -p "$output_dir"
else
    output_dir="$(mktemp -d "${TMPDIR:-/tmp}/apolysis-production-hardening-service-mesh-live.XXXXXX")"
fi

stamp="$(date +%Y%m%d%H%M%S)-$$"
istio_chart_version="${APOLYSIS_PRODUCTION_HARDENING_ISTIO_CHART_VERSION:-1.30.1}"
istio_pilot_cpu_request="${APOLYSIS_PRODUCTION_HARDENING_ISTIO_PILOT_CPU_REQUEST:-100m}"
istio_pilot_memory_request="${APOLYSIS_PRODUCTION_HARDENING_ISTIO_PILOT_MEMORY_REQUEST:-256Mi}"
istio_pilot_cpu_limit="${APOLYSIS_PRODUCTION_HARDENING_ISTIO_PILOT_CPU_LIMIT:-500m}"
istio_pilot_memory_limit="${APOLYSIS_PRODUCTION_HARDENING_ISTIO_PILOT_MEMORY_LIMIT:-512Mi}"
namespace="apolysis-production-hardening-mesh-$stamp"
plaintext_namespace="apolysis-production-hardening-plain-$stamp"
evidence_path="$output_dir/apolysis-production-hardening-istio-live-evidence.json"
report_path="$output_dir/apolysis-production-hardening-istio-live-evidence-report.json"
...
cleanup() {
    ...
    if [[ "$installed_istio" == "1" && "${APOLYSIS_PRODUCTION_HARDENING_KEEP_ISTIO:-0}" != "1" ]]; then
...
install_istio_if_missing() {
    ...
    if kubectl get namespace istio-system >/dev/null 2>&1; then
        echo "apolysis-production-hardening: istio-system exists but required Istio CRDs/control-plane are missing; refusing to mutate it" >&2
        exit 1
    fi
...
metadata:
  ...
  labels:
    ...
    apolysis.dev/validation: production-hardening-service-mesh-live
---
apiVersion: v1
kind: ServiceAccount
metadata:
    ...
    spec:
      ...
      containers:
        - name: server
          ...
          command: ["/bin/sh", "-c"]
          args:
            - mkdir -p /www && echo apolysis-production-hardening-mesh-ok >/www/index.html && httpd -f -p 8080 -h /www
          ports:
            - containerPort: 8080
          securityContext:
...
grep -q 'apolysis-production-hardening-mesh-ok' "$authorized_output" || {
    echo "apolysis-production-hardening: authorized mTLS client did not receive expected response" >&2
    cat "$authorized_output" >&2 || true
    exit 1
}

if exec_client_probe "$namespace" unauthorized "$unauthorized_output" "$target_url"; then
    echo "apolysis-production-hardening: unauthorized mTLS client unexpectedly reached the server" >&2
    cat "$unauthorized_output" >&2 || true
    exit 1
fi

if exec_client_probe "$plaintext_namespace" plaintext "$plaintext_output" "$target_url"; then
    echo "apolysis-production-hardening: plaintext client unexpectedly reached the strict-mTLS server" >&2
...
evidence = {
    "evidence_id": f"production-hardening-istio-mtls-handshake-{namespace}",
    "source": "live_cluster",
    "provider": "istio",
...
cargo run -q -p apolysis-validation --bin apolysis-production-hardening-service-mesh-live-evidence -- \
    --evidence "$evidence_path" >"$report_path"

jq -e '.passed == true and .approval.provider == "istio"' "$report_path" >/dev/null

printf 'apolysis-production-hardening: live Istio service-mesh validation passed (%s)\n' "$output_dir"
