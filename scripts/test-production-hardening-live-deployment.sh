...
confirm="${APOLYSIS_CONFIRM_PRODUCTION_HARDENING_LIVE_DEPLOYMENT:-0}"

if [[ "$confirm" != "1" ]]; then
    cat >&2 <<'EOF'
apolysis-production-hardening: refusing to run live DaemonSet deployment without confirmation.
Set APOLYSIS_CONFIRM_PRODUCTION_HARDENING_LIVE_DEPLOYMENT=1 to build/import a local image,
deploy the ProductionHardening production baseline to k3s, collect health/log evidence, and
...
if kubectl get namespace apolysis-system >/dev/null 2>&1; then
    echo "apolysis-production-hardening: namespace apolysis-system already exists; refusing to overwrite it" >&2
    exit 1
fi

if [[ -n "${APOLYSIS_PRODUCTION_HARDENING_LIVE_DEPLOYMENT_OUTPUT_DIR:-}" ]]; then
    output_dir="$APOLYSIS_PRODUCTION_HARDENING_LIVE_DEPLOYMENT_OUTPUT_DIR"
    mkdir -p "$output_dir"
else
    output_dir="$(mktemp -d "${TMPDIR:-/tmp}/apolysis-production-hardening-live-deployment.XXXXXX")"
fi

stamp="$(date +%Y%m%d%H%M%S)-$$"
image="localhost/apolysisd:production-hardening-live-$stamp"
...
workload_manifest="$output_dir/apolysis-production-hardening-live-workload.apply.yaml"
restart_workload_manifest="$output_dir/apolysis-production-hardening-restart-workload.apply.yaml"
socket_recovery_workload_manifest="$output_dir/apolysis-production-hardening-socket-recovery-workload.apply.yaml"
queue_pressure_workload_manifest="$output_dir/apolysis-production-hardening-queue-pressure-workload.apply.yaml"
unwritable_store_workload_manifest="$output_dir/apolysis-production-hardening-unwritable-store-workload.apply.yaml"
unwritable_store_recovery_workload_manifest="$output_dir/apolysis-production-hardening-unwritable-store-recovery-workload.apply.yaml"
state_path="/tmp/apolysis-production-hardening-live-state-$stamp"
session_id="apolysis-production-hardening-live"
applied=0
port_forward_pid=""
...
wait_for_daemon_pod() {
    ...
    echo "apolysis-production-hardening: daemon pod did not become ready after rollout" >&2
    if [[ -n "$previous" ]]; then
        echo "apolysis-production-hardening: previous daemon pod was $previous" >&2
    fi
...
PY
    ...
    echo "apolysis-production-hardening: daemon health did not reach $expected_adapter=$expected_state" >&2
...
PY
    ...
    if [[ "$metrics_ready" != "1" ]]; then
        echo "apolysis-production-hardening: metrics scrape did not become ready for $artifact_prefix" >&2
...
PY
    ...
    echo "apolysis-production-hardening: queue pressure metrics did not reach accepted threshold" >&2
...
wait_for_session_timeline_record() {
    ...
    echo "apolysis-production-hardening: session timeline did not contain expected pattern '$pattern' for $session_id" >&2
...
PY
    ...
    echo "apolysis-production-hardening: daemon health did not reach storage=$expected_storage" >&2
    cat "$health_output" >&2 || true
    cat "$health_error" >&2 || true
    return 1
}
...
write_marked_workload_manifest \
    apolysis-production-hardening-live-workload \
    "$session_id" \
    "$workload_manifest"
...
kubectl -n apolysis-system wait \
    --for=condition=Ready \
    pod/apolysis-production-hardening-live-workload \
    --timeout=120s
...
kubectl -n apolysis-system get pod apolysis-production-hardening-live-workload -o yaml \
    >"$output_dir/apolysis-production-hardening-live-workload.yaml"
kubectl get events -n apolysis-system --sort-by=.lastTimestamp >"$output_dir/kubernetes-events.txt"

if [[ "$health_ready" != "1" ]]; then
    echo "apolysis-production-hardening: daemon health did not reach ProductionHardening live readiness" >&2
...
if [[ "$metrics_ready" != "1" ]]; then
    echo "apolysis-production-hardening: metrics scrape did not become ready" >&2
...
for forbidden in [
    ...
    if forbidden in logs:
        raise SystemExit(f"apolysis-production-hardening: live deployment log contains forbidden text: {forbidden}")
PY

restart_session_id="apolysis-production-hardening-restart"
...
write_marked_workload_manifest \
    apolysis-production-hardening-restart-workload \
    "$restart_session_id" \
    "$restart_workload_manifest"
kubectl apply -f "$restart_workload_manifest"
kubectl -n apolysis-system wait \
    --for=condition=Ready \
    pod/apolysis-production-hardening-restart-workload \
    --timeout=120s
...
kubectl -n apolysis-system get pod apolysis-production-hardening-restart-workload -o yaml \
    >"$output_dir/apolysis-production-hardening-restart-workload.yaml"
...
bad_socket = "/host/run/k3s/containerd/apolysis-production-hardening-missing-k3s-containerd.sock"
...
socket_recovery_session_id="apolysis-production-hardening-socket-recovery"
...
write_marked_workload_manifest \
    apolysis-production-hardening-socket-recovery-workload \
    "$socket_recovery_session_id" \
    "$socket_recovery_workload_manifest"
kubectl apply -f "$socket_recovery_workload_manifest"
kubectl -n apolysis-system wait \
    --for=condition=Ready \
    pod/apolysis-production-hardening-socket-recovery-workload \
    --timeout=120s
...
kubectl -n apolysis-system get pod apolysis-production-hardening-socket-recovery-workload -o yaml \
    >"$output_dir/apolysis-production-hardening-socket-recovery-workload.yaml"
kubectl get events -n apolysis-system --sort-by=.lastTimestamp \
    >"$output_dir/kubernetes-events-after-resilience.txt"

queue_pressure_session_id="apolysis-production-hardening-queue-pressure-$stamp"
write_marked_workload_manifest \
    apolysis-production-hardening-queue-pressure-workload \
...
kubectl -n apolysis-system wait \
    --for=condition=Ready \
    pod/apolysis-production-hardening-queue-pressure-workload \
    --timeout=120s

wait_for_session_timeline_record \
    "$queue_pressure_session_id" \
    "cgroup_discovered" \
    "$output_dir/apolysis-production-hardening-queue-pressure-timeline.jsonl"
...
kubectl -n apolysis-system get pod apolysis-production-hardening-queue-pressure-workload -o yaml \
    >"$output_dir/apolysis-production-hardening-queue-pressure-workload.yaml"

unwritable_store_session_id="apolysis-production-hardening-unwritable-store-$stamp"
...
write_marked_workload_manifest \
    apolysis-production-hardening-unwritable-store-workload \
    "$unwritable_store_session_id" \
    "$unwritable_store_workload_manifest"
kubectl apply -f "$unwritable_store_workload_manifest"
kubectl -n apolysis-system wait \
    --for=condition=Ready \
    pod/apolysis-production-hardening-unwritable-store-workload \
    --timeout=120s
...
kubectl -n apolysis-system get pod apolysis-production-hardening-unwritable-store-workload -o yaml \
    >"$output_dir/apolysis-production-hardening-unwritable-store-workload.yaml"

kubectl -n apolysis-system delete pod/apolysis-production-hardening-unwritable-store-workload \
...
unwritable_store_recovery_session_id="apolysis-production-hardening-unwritable-store-recovery-$stamp"
write_marked_workload_manifest \
    apolysis-production-hardening-unwritable-store-recovery-workload \
...
kubectl -n apolysis-system wait \
    --for=condition=Ready \
    pod/apolysis-production-hardening-unwritable-store-recovery-workload \
    --timeout=120s
...
kubectl -n apolysis-system get pod apolysis-production-hardening-unwritable-store-recovery-workload -o yaml \
    >"$output_dir/apolysis-production-hardening-unwritable-store-recovery-workload.yaml"
kubectl get events -n apolysis-system --sort-by=.lastTimestamp \
...
if kubectl get namespace apolysis-system >/dev/null 2>&1; then
    echo "apolysis-production-hardening: namespace apolysis-system still exists after cleanup" >&2
    exit 1
fi

echo "apolysis-production-hardening: live deployment validation passed; artifacts: $output_dir"
