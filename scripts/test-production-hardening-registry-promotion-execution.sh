...
output_dir="${APOLYSIS_PRODUCTION_HARDENING_REGISTRY_PROMOTION_EXECUTION_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/production-hardening-registry-promotion-execution.XXXXXX")}"
mkdir -p "$output_dir"
output_dir="$(cd "$output_dir" && pwd)"
registry_image="${APOLYSIS_PRODUCTION_HARDENING_REGISTRY_IMAGE:-registry:2}"
registry_name="${APOLYSIS_PRODUCTION_HARDENING_REGISTRY_EXECUTION_NAME:-apolysis-production-hardening-promotion-$stamp}"
source_image=""
repository="${APOLYSIS_PRODUCTION_HARDENING_REGISTRY_EXECUTION_REPOSITORY:-apolysisd}"
source_tag="${APOLYSIS_PRODUCTION_HARDENING_REGISTRY_EXECUTION_SOURCE_TAG:-staging-$stamp}"
target_tag="${APOLYSIS_PRODUCTION_HARDENING_REGISTRY_EXECUTION_TARGET_TAG:-prod-$stamp}"
rollback_tag="${APOLYSIS_PRODUCTION_HARDENING_REGISTRY_EXECUTION_ROLLBACK_TAG:-prod-previous-$stamp}"
retention_days="${APOLYSIS_PRODUCTION_HARDENING_REGISTRY_EXECUTION_RETENTION_DAYS:-180}"
evidence="$output_dir/apolysis-production-hardening-registry-promotion-execution-evidence.json"
report="$output_dir/apolysis-production-hardening-registry-promotion-execution-report.json"
fail_evidence="$output_dir/apolysis-production-hardening-registry-promotion-execution-evidence-fail.json"
fail_report="$output_dir/apolysis-production-hardening-registry-promotion-execution-report-fail.json"
...
if ! docker image inspect "$registry_image" >/dev/null 2>&1; then
    echo "apolysis-production-hardening: missing registry image: $registry_image" >&2
    echo "apolysis-production-hardening: pull it first or set APOLYSIS_PRODUCTION_HARDENING_REGISTRY_IMAGE to an available image" >&2
    exit 1
fi

registry_port="${APOLYSIS_PRODUCTION_HARDENING_REGISTRY_EXECUTION_PORT:-$(choose_free_port)}"
...
observations="$output_dir/apolysis-production-hardening-registry-promotion-api-observations.json"

mkdir -p "$image_context"
printf 'apolysis production_hardening registry promotion execution %s\n' "$stamp" >"$image_context/payload.txt"
cat >"$image_context/Dockerfile" <<'DOCKERFILE'
FROM scratch
LABEL org.opencontainers.image.title="apolysis-production-hardening-registry-promotion-execution"
...
if ! curl -fsS "$registry_url/v2/" >/dev/null 2>&1; then
    echo "apolysis-production-hardening: local registry did not become ready" >&2
...
if [[ ! "$image_digest" =~ ^sha256:[0-9a-f]{64}$ ]]; then
    echo "apolysis-production-hardening: registry did not return a valid staging digest" >&2
    exit 1
fi
if [[ -z "$manifest_media_type" ]]; then
    echo "apolysis-production-hardening: registry did not return a manifest content type" >&2
    exit 1
fi
...
if [[ "$promotion_status" != "201" && "$promotion_status" != "202" ]]; then
    echo "apolysis-production-hardening: production manifest promotion failed with HTTP $promotion_status" >&2
    exit 1
fi
...
if [[ "$rollback_status" != "201" && "$rollback_status" != "202" ]]; then
    echo "apolysis-production-hardening: rollback manifest promotion failed with HTTP $rollback_status" >&2
    exit 1
fi
...
evidence = {
    "evidence_id": "production-hardening-registry-promotion-execution",
...
if cargo run -q -p apolysis-validation --bin apolysis-production-hardening-registry-promotion-execution-evidence -- \
    --evidence "$fail_evidence" >"$fail_report"; then
    echo "apolysis-production-hardening: invalid registry promotion execution evidence unexpectedly passed" >&2
    exit 1
fi
...
printf 'apolysis-production-hardening: registry promotion execution gate passed (%s)\n' "$output_dir"
