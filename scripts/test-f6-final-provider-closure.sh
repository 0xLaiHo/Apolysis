...
output_dir="${APOLYSIS_F6_FINAL_PROVIDER_CLOSURE_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/f6-final-provider-closure.XXXXXX")}"
mkdir -p "$output_dir"
output_dir="$(cd "$output_dir" && pwd)"

report="$output_dir/apolysis-f6-final-provider-closure-report.json"
require_ready="${APOLYSIS_REQUIRE_F6_FINAL_PROVIDER_CLOSURE:-0}"
...
f5_gate = repo_root / "scripts/test-f5-final-provider-closure.sh"
downstream_dir = output_dir / "f5-final-provider-closure"
downstream_report_path = downstream_dir / "apolysis-f5-final-provider-closure-report.json"
...
requested_source = (
    env_value("APOLYSIS_F6_PROVIDER_ARTIFACT_SOURCE", "APOLYSIS_F6_ARTIFACT_SOURCE") or "auto"
)
...
env = os.environ.copy()
env["APOLYSIS_F5_FINAL_PROVIDER_CLOSURE_OUTPUT_DIR"] = str(downstream_dir)
env["APOLYSIS_REQUIRE_F5_FINAL_PROVIDER_CLOSURE"] = "0"
env["APOLYSIS_RUN_F5_FINAL_PROVIDER_COMPLETION"] = "1" if run_final_closure else "0"
...
downstream_output_path = output_dir / "f5-final-provider-closure.out"
...
if process.returncode != 0:
    missing_requirements.append("f5_final_provider_closure_audit_succeeded")
...
report = {
    ...
    "notes": [
        ...
        "F6.4 maps F6 provider artifact source controls and final-closure execution controls to the historical F5.47 closure gate.",
        "Default audit mode does not dispatch GitHub workflows and does not run final provider completion unless APOLYSIS_RUN_F6_FINAL_PROVIDER_CLOSURE=1 is set.",
    ...
    "next_commands": {
        "audit": "./scripts/test-f6-final-provider-closure.sh",
        "required_from_local_artifacts": (
            ...
            "APOLYSIS_REQUIRE_F6_FINAL_PROVIDER_CLOSURE=1 ./scripts/test-f6-final-provider-closure.sh"
        ),
        "download_then_close": (
...
if require_ready and not final_provider_closure_ready:
    print(f"apolysis-f6: final provider closure failed closed ({report_path})", file=sys.stderr)
    print("missing requirements: " + ", ".join(report["missing_requirements"]), file=sys.stderr)
    raise SystemExit(1)
PY

cat <<EOF
apolysis-f6: final provider closure audit written ($output_dir)
APOLYSIS_F6_FINAL_PROVIDER_CLOSURE_REPORT=$report
EOF
