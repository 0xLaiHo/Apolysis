...
restart_runtime_services() {
    ...
    start_host_unit "$unit" /bin/bash -lc \
        "systemctl restart containerd.service docker.service k3s.service"
    wait_for_services
}
...
host_bash 'crictl --config /dev/null --runtime-endpoint unix:///run/containerd/containerd.sock --image-endpoint unix:///run/containerd/containerd.sock info | jq -e '\''.config.containerd.runtimes | has("runsc") | not and has("kata") | not'\'''
host_bash 'crictl --runtime-endpoint unix:///run/k3s/containerd/containerd.sock info | jq -e '\''.config.containerd.runtimes | has("runsc") | not and has("kata") | not'\'''
