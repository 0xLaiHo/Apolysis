...
output_dir="${APOLYSIS_PRODUCTION_HARDENING_PROMOTION_POLICY_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/production-hardening-release-promotion-policy.XXXXXX")}"
mkdir -p "$output_dir"
...
request="$output_dir/apolysis-production-hardening-release-promotion-request.json"
pass_report="$output_dir/apolysis-production-hardening-release-promotion-policy-pass.json"
fail_request="$output_dir/apolysis-production-hardening-release-promotion-request-fail.json"
fail_report="$output_dir/apolysis-production-hardening-release-promotion-policy-fail.json"
...
cat >"$release_manifest" <<'JSON'
{
  ...
  "files": [
    {"path": "apolysis-production-hardening-release-payload.tar.gz", "sha256": "3333333333333333333333333333333333333333333333333333333333333333", "size": 1},
    {"path": "apolysis-production-hardening-apolysisd-image.tar", "sha256": "4444444444444444444444444444444444444444444444444444444444444444", "size": 1},
    {"path": "apolysis-production-hardening-sbom.cdx.json", "sha256": "5555555555555555555555555555555555555555555555555555555555555555", "size": 1},
    {"path": "apolysis-production-hardening-provenance.intoto.json", "sha256": "6666666666666666666666666666666666666666666666666666666666666666", "size": 1}
  ]
}
JSON
...
source_tag="production-hardening-registry-20260624"
...
cargo run -q -p apolysis-validation --bin apolysis-production-hardening-release-promotion-policy -- \
...
if cargo run -q -p apolysis-validation --bin apolysis-production-hardening-release-promotion-policy -- \
    --release-manifest "$release_manifest" \
    --registry-attachment "$registry_attachment" \
    --archive-manifest "$archive_manifest" \
    --request "$fail_request" >"$fail_report"; then
    echo "apolysis-production-hardening: invalid production promotion request unexpectedly passed" >&2
    exit 1
fi
...
printf 'apolysis-production-hardening: release promotion policy gate passed (%s)\n' "$output_dir"
