...
confirm="${APOLYSIS_CONFIRM_PRODUCTION_HARDENING_MANAGED_CLOUD_SERVICE_MESH:-0}"

if [[ "$confirm" != "1" ]]; then
    cat >&2 <<'EOF'
apolysis-production-hardening: managed Cloud Service Mesh qualification is opt-in.
Set APOLYSIS_CONFIRM_PRODUCTION_HARDENING_MANAGED_CLOUD_SERVICE_MESH=1 after confirming the GKE
...
fleet_project="$(require_env APOLYSIS_PRODUCTION_HARDENING_GKE_MESH_FLEET_PROJECT)"
membership="$(require_env APOLYSIS_PRODUCTION_HARDENING_GKE_MESH_MEMBERSHIP)"
membership_location="${APOLYSIS_PRODUCTION_HARDENING_GKE_MESH_MEMBERSHIP_LOCATION:-global}"
cluster_name="${APOLYSIS_PRODUCTION_HARDENING_GKE_MESH_CLUSTER_NAME:-$membership}"
revision="${APOLYSIS_PRODUCTION_HARDENING_GKE_MESH_REVISION:-asm-managed}"

mkdir -p "$repo_root/target"
output_dir="${APOLYSIS_PRODUCTION_HARDENING_MANAGED_CLOUD_SERVICE_MESH_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/production-hardening-managed-cloud-service-mesh.XXXXXX")}"
mkdir -p "$output_dir"
...
evidence="$output_dir/apolysis-production-hardening-managed-cloud-service-mesh-evidence.json"
report="$output_dir/apolysis-production-hardening-managed-cloud-service-mesh-report.json"
...
if [[ -z "$state_json" || "$state_json" == "null" ]]; then
    echo "apolysis-production-hardening: membership state was not found in fleet mesh describe output" >&2
    exit 1
fi
if [[ -z "$spec_json" || "$spec_json" == "null" ]]; then
    echo "apolysis-production-hardening: membership spec was not found in fleet mesh describe output" >&2
    exit 1
fi
...
if [[ "$management" != "MANAGEMENT_AUTOMATIC" ]]; then
    echo "apolysis-production-hardening: managed Cloud Service Mesh must use MANAGEMENT_AUTOMATIC" >&2
    exit 1
fi
if [[ "$control_plane_state" != "ACTIVE" ]]; then
    echo "apolysis-production-hardening: managed Cloud Service Mesh controlPlaneManagement must be ACTIVE" >&2
    exit 1
fi
if [[ "$control_plane_implementation" != "ISTIOD" && "$control_plane_implementation" != "TRAFFIC_DIRECTOR" ]]; then
    echo "apolysis-production-hardening: managed Cloud Service Mesh implementation must be ISTIOD or TRAFFIC_DIRECTOR" >&2
    exit 1
fi
if [[ "$data_plane_state" == "NEEDS_ATTENTION" || "$data_plane_state" == "STALLED" || "$data_plane_state" == "FAILED_PRECONDITION" ]]; then
    echo "apolysis-production-hardening: managed Cloud Service Mesh data plane is not healthy: $data_plane_state" >&2
    exit 1
fi
...
' "$control_plane_revision" >/dev/null || {
    echo "apolysis-production-hardening: no managed Cloud Service Mesh controlplanerevision evidence found" >&2
    exit 1
}
...
if [[ "$managed_namespace_count" -lt 1 && "${APOLYSIS_PRODUCTION_HARDENING_GKE_MESH_ALLOW_EMPTY_NAMESPACE_SET:-0}" != "1" ]]; then
    echo "apolysis-production-hardening: no namespaces are labeled for managed Cloud Service Mesh injection" >&2
    exit 1
fi
...
evidence = {
    "evidence_id": f"production-hardening-managed-cloud-service-mesh-{observed_at_unix_ms}",
...
cat <<EOF
apolysis-production-hardening: managed Cloud Service Mesh qualification passed ($output_dir)
