...
cargo test -p apolysis-validation --test runtime_guardrails_runtime_guardrail_matrix
cargo test -p apolysis-validation --test runtime_guardrails_runtime_adapter_evidence
cargo test -p apolysis-validation --test runtime_guardrails_gvisor_metadata_evidence
cargo test -p apolysis-validation --test runtime_guardrails_kubernetes_agent_sandbox_evidence
cargo test -p apolysis-validation --test runtime_guardrails_kata_boundary_evidence
cargo test -p apolysis-daemon --test runtime_adapters runtime_workload_becomes_runtime_guardrails_runtime_adapter_evidence
cargo test -p apolysis-visibility --test runtime_guardrails_gvisor_metadata
cargo test -p apolysis-visibility --test runtime_guardrails_kata_boundary
cargo test -p apolysis-kubernetes --test runtime_guardrails_agent_sandbox_evidence
cargo run -p apolysis-validation --bin apolysis-runtime-guardrails-runtime-guardrail-matrix \
  < tests/fixtures/validation/runtime-guardrails-runtime-guardrail-local-live.json \
  > /tmp/apolysis-runtime-guardrails-runtime-guardrail-matrix.json
cargo run -p apolysis-validation --bin apolysis-runtime-guardrails-runtime-guardrail-matrix \
  < tests/fixtures/validation/runtime-guardrails-runtime-guardrail-request.json \
  > /tmp/apolysis-runtime-guardrails-runtime-guardrail-adapter-matrix.json

live_bundle_artifacts="$(mktemp -d "${TMPDIR:-/tmp}/apolysis-runtime-guardrails-live-runtime-evidence.XXXXXX")"
live_bundle_output="$(mktemp -d "${TMPDIR:-/tmp}/apolysis-runtime-guardrails-live-runtime-evidence-output.XXXXXX")"
for artifact in \
...
python - "$live_bundle_artifacts/runtime-guardrails-runtime-adapter-evidence.jsonl" <<'PY'
import json
import sys
...
PY
python - "$live_bundle_artifacts/runtime-guardrails-gvisor-metadata-evidence.jsonl" "$live_bundle_artifacts/runtime-guardrails-kubernetes-agent-sandbox-evidence.jsonl" "$live_bundle_artifacts/runtime-guardrails-kata-boundary-evidence.jsonl" <<'PY'
import json
import sys
from pathlib import Path
...
APOLYSIS_RUNTIME_GUARDRAILS_LIVE_RUNTIME_EVIDENCE_OUTPUT_DIR="$live_bundle_output" \
  ./scripts/write-runtime-guardrails-live-runtime-evidence-bundle.sh

test -s "$live_bundle_output/runtime-guardrails-live-runtime-evidence-request.json"
test -s "$live_bundle_output/runtime-guardrails-live-runtime-evidence-report.json"
python - "$live_bundle_output/runtime-guardrails-live-runtime-evidence-request.json" <<'PY'
import json
import sys
from pathlib import Path
...
restore_check_root="$(mktemp -d "${TMPDIR:-/tmp}/apolysis-runtime-foundation-restore-check-root.XXXXXX")"
restore_check_artifacts="$(mktemp -d "${TMPDIR:-/tmp}/apolysis-runtime-foundation-restore-check-artifacts.XXXXXX")"
...
APOLYSIS_HOST_ROOT="$restore_check_root" \
  ./scripts/assert-runtime-foundation-runtime-matrix-restored.sh
...
if APOLYSIS_RUNTIME_ADAPTER_MATRIX_OUTPUT_DIR="$restore_check_artifacts" \
  APOLYSIS_HOST_ROOT="$restore_check_root" \
  ./scripts/assert-runtime-foundation-runtime-matrix-restored.sh; then
  echo "apolysis-runtime_guardrails: restore manifest check accepted a stale runtime drop-in" >&2
  exit 1
fi

python - "$live_bundle_output/runtime-guardrails-live-runtime-evidence-report.json" <<'PY'
import json
import sys
from pathlib import Path

report = json.loads(Path("/tmp/apolysis-runtime-guardrails-runtime-guardrail-matrix.json").read_text())
...
adapter_report = json.loads(Path("/tmp/apolysis-runtime-guardrails-runtime-guardrail-adapter-matrix.json").read_text())
...
echo "apolysis-runtime_guardrails: runtime guardrail support matrix validation passed"
