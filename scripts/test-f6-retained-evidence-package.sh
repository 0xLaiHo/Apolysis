...
output_dir="${APOLYSIS_F6_RETAINED_EVIDENCE_PACKAGE_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/f6-retained-evidence-package.XXXXXX")}"
mkdir -p "$output_dir"
output_dir="$(cd "$output_dir" && pwd)"

report="$output_dir/apolysis-f6-retained-evidence-package-report.json"
require_ready="${APOLYSIS_REQUIRE_F6_RETAINED_EVIDENCE_PACKAGE:-0}"
...
source_report_value = env_value("APOLYSIS_F6_EVIDENCE_PACKAGE_REPORT")
source_manifest_value = env_value("APOLYSIS_F6_EVIDENCE_PACKAGE_MANIFEST")
source_archive_value = env_value("APOLYSIS_F6_EVIDENCE_PACKAGE_ARCHIVE")
source_sha_value = env_value("APOLYSIS_F6_EVIDENCE_PACKAGE_SHA256")
retention_root_value = env_value("APOLYSIS_F6_RETAINED_EVIDENCE_PACKAGE_ROOT")
...
retention_provider = env_value("APOLYSIS_F6_EVIDENCE_RETENTION_PROVIDER") or "local_filesystem"
retention_mode = env_value("APOLYSIS_F6_EVIDENCE_RETENTION_MODE") or "local_copy"
retention_uri = env_value("APOLYSIS_F6_EVIDENCE_RETENTION_URI")
retention_control_plane = env_value("APOLYSIS_F6_EVIDENCE_RETENTION_CONTROL_PLANE")

retention_root = Path(retention_root_value) if retention_root_value else None
raw_package_id = source_manifest_doc.get("package_id") or source_report_doc.get("package_id") or "f6-evidence-package"
package_id = re.sub(r"[^A-Za-z0-9_.-]", "_", str(raw_package_id)) or "f6-evidence-package"
retained_dir = retention_root / str(package_id) if retention_root is not None else output_dir / "retained-package"
retained_archive = retained_dir / "apolysis-f6-evidence-package.tar.gz"
retained_sha = retained_dir / "apolysis-f6-evidence-package.tar.gz.sha256"
retained_report = retained_dir / "apolysis-f6-evidence-package-report.json"
retained_manifest = retained_dir / "apolysis-f6-evidence-package-manifest.json"
retention_manifest = retained_dir / "apolysis-f6-retained-evidence-package-manifest.json"
...
if retention_root is not None and not missing_requirements:
    ...
    retention_doc = {
        ...
        "source": "f6_retained_evidence_package",
...
report = {
    ...
    "notes": [
        ...
        "The F6.7 gate validates a previously generated F6.6 evidence package before copying it into a retained package directory.",
        "The default retention provider is local_filesystem; external WORM or object-lock metadata can be supplied with APOLYSIS_F6_EVIDENCE_RETENTION_* variables.",
    ...
    "next_commands": {
        "audit": "./scripts/test-f6-retained-evidence-package.sh",
        "required_local_retention": "APOLYSIS_F6_EVIDENCE_PACKAGE_REPORT=<package-report> APOLYSIS_F6_RETAINED_EVIDENCE_PACKAGE_ROOT=<retention-root> APOLYSIS_REQUIRE_F6_RETAINED_EVIDENCE_PACKAGE=1 ./scripts/test-f6-retained-evidence-package.sh",
    },
...
if require_ready and not retained_evidence_package_ready:
    print(f"apolysis-f6: retained evidence package failed closed ({report_path})", file=sys.stderr)
    print("missing requirements: " + ", ".join(report["missing_requirements"]), file=sys.stderr)
    raise SystemExit(1)
PY

cat <<EOF
apolysis-f6: retained evidence package audit written ($output_dir)
APOLYSIS_F6_RETAINED_EVIDENCE_PACKAGE_REPORT=$report
EOF
