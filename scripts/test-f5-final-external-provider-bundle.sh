...
output_dir="${APOLYSIS_F5_FINAL_EXTERNAL_BUNDLE_TEST_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/f5-final-external-provider-bundle-test.XXXXXX")}"
mkdir -p "$output_dir"
...
APOLYSIS_F5_FINAL_EXTERNAL_BUNDLE_OUTPUT_DIR="$success_dir" \
...
APOLYSIS_F5_MANAGED_MESH_REPORT="$managed_mesh_report" \
    "$repo_root/scripts/build-f5-final-external-provider-bundle.sh"

report="$success_dir/apolysis-f5-final-external-provider-bundle-report.json"
bundle="$success_dir/bundle-root/apolysis-f5-final-external-provider-bundle.json"
jq -e '
  .passed == true
...
if APOLYSIS_F5_FINAL_EXTERNAL_BUNDLE_OUTPUT_DIR="$failure_dir" \
APOLYSIS_F5_SIGNING_PROVIDER="aws_kms" \
APOLYSIS_F5_SIGNING_CONTROL_PLANE="awskms://arn:aws:kms:us-west-2:111122223333:key/apolysis-f5" \
APOLYSIS_F5_SIGNING_EVIDENCE="$signing_evidence" \
APOLYSIS_F5_SIGNING_REPORT="$signing_report" \
...
APOLYSIS_F5_MANAGED_MESH_CONTROL_PLANE="gke:prod-us-central1:anthos-service-mesh" \
APOLYSIS_F5_MANAGED_MESH_REPORT="$managed_mesh_report" \
    "$repo_root/scripts/build-f5-final-external-provider-bundle.sh" >"$failure_dir/missing-managed-mesh.out" 2>"$failure_dir/missing-managed-mesh.err"; then
    echo "apolysis-f5: final external bundle unexpectedly passed without managed mesh evidence" >&2
    exit 1
fi

grep -q 'APOLYSIS_F5_MANAGED_MESH_EVIDENCE is required' "$failure_dir/missing-managed-mesh.err" || {
    echo "apolysis-f5: final external bundle did not fail closed on missing managed mesh evidence" >&2
    exit 1
}

printf 'apolysis-f5: final external provider bundle assembly gate passed (%s)\n' "$output_dir"
