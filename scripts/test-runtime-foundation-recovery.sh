...
run_exact_test() {
    ...
    if ! cargo test -p "$package" --test "$test_target" "$test_name" \
        ...
        echo "apolysis-runtime_foundation: recovery test not found: $package/$test_target::$test_name" >&2
        exit 1
    fi
...
./scripts/test-runtime-foundation-apolysisd-systemd.sh

echo "apolysis-runtime_foundation: recovery qualification passed"
