...
output_dir="${APOLYSIS_RUNTIME_ADAPTER_MATRIX_OUTPUT_DIR:-$(mktemp -d "${TMPDIR:-/tmp}/apolysis-runtime-foundation-runtime-adapter-matrix.XXXXXX")}"
applied=0

if [[ "${APOLYSIS_CONFIRM_RUNTIME_ADAPTER_MATRIX:-0}" != "1" ]]; then
    cat >&2 <<'EOF'
apolysis-runtime_foundation: runtime adapter matrix test is disabled by default because it
...
runtime_guardrails_runtime_adapter_evidence_output="$output_dir/runtime-guardrails-runtime-adapter-evidence.jsonl"
runtime_guardrails_gvisor_metadata_evidence_output="$output_dir/runtime-guardrails-gvisor-metadata-evidence.jsonl"
runtime_guardrails_kubernetes_agent_sandbox_evidence_output="$output_dir/runtime-guardrails-kubernetes-agent-sandbox-evidence.jsonl"
runtime_guardrails_kata_boundary_evidence_output="$output_dir/runtime-guardrails-kata-boundary-evidence.jsonl"
...
APOLYSIS_RUNTIME_GUARDRAILS_RUNTIME_ADAPTER_EVIDENCE_OUTPUT="$runtime_guardrails_runtime_adapter_evidence_output" \
APOLYSIS_RUNTIME_GUARDRAILS_GVISOR_METADATA_EVIDENCE_OUTPUT="$runtime_guardrails_gvisor_metadata_evidence_output" \
APOLYSIS_RUNTIME_GUARDRAILS_KUBERNETES_AGENT_SANDBOX_EVIDENCE_OUTPUT="$runtime_guardrails_kubernetes_agent_sandbox_evidence_output" \
APOLYSIS_RUNTIME_GUARDRAILS_KATA_BOUNDARY_EVIDENCE_OUTPUT="$runtime_guardrails_kata_boundary_evidence_output" \
...
test -s "$runtime_guardrails_runtime_adapter_evidence_output"
test -s "$runtime_guardrails_gvisor_metadata_evidence_output"
test -s "$runtime_guardrails_kubernetes_agent_sandbox_evidence_output"
test -s "$runtime_guardrails_kata_boundary_evidence_output"
...
if [[ "${EUID:-$(id -u)}" -eq 0 ]]; then
    ...
    host_bash "cd '$repo_root' && APOLYSIS_RUNTIME_ADAPTER_MATRIX_OUTPUT_DIR='$output_dir' ./scripts/assert-runtime-foundation-runtime-matrix-restored.sh"
fi
...
test -s "$output_dir/runtime-guardrails-live-runtime-evidence-request.json"
test -s "$output_dir/runtime-guardrails-live-runtime-evidence-report.json"

echo "apolysis-runtime_foundation: runtime adapter matrix passed; artifacts kept at $output_dir"
