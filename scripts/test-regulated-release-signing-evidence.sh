...
output_dir="${APOLYSIS_REGULATED_RELEASE_SIGNING_EVIDENCE_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/regulated-release-signing-evidence.XXXXXX")}"
mkdir -p "$output_dir"
output_dir="$(cd "$output_dir" && pwd)"

report="$output_dir/apolysis-regulated-release-signing-evidence-report.json"
require_ready="${APOLYSIS_REQUIRE_REGULATED_RELEASE_SIGNING_EVIDENCE:-0}"
...
production_hardening_gate = repo_root / "scripts/test-production-hardening-signing-provider-readiness.sh"
downstream_dir = output_dir / "production-hardening-signing-provider-readiness"
downstream_report_path = downstream_dir / "apolysis-production-hardening-signing-provider-readiness-report.json"
...
requested_provider = env_value("APOLYSIS_REGULATED_RELEASE_SIGNING_PROVIDER") or "auto"
signing_evidence = env_value("APOLYSIS_REGULATED_RELEASE_SIGNING_EVIDENCE", "APOLYSIS_PRODUCTION_HARDENING_SIGNING_EVIDENCE")
signing_report = env_value("APOLYSIS_REGULATED_RELEASE_SIGNING_REPORT", "APOLYSIS_PRODUCTION_HARDENING_SIGNING_REPORT")

provider_errors: list[str] = []
if requested_provider not in allowed_providers:
    provider_errors.append("APOLYSIS_REGULATED_RELEASE_SIGNING_PROVIDER")

env = os.environ.copy()
env["APOLYSIS_PRODUCTION_HARDENING_SIGNING_PROVIDER_READINESS_OUTPUT_DIR"] = str(downstream_dir)
env["APOLYSIS_REQUIRE_PRODUCTION_HARDENING_SIGNING_PROVIDER_READINESS"] = "0"

if signing_evidence:
    env["APOLYSIS_PRODUCTION_HARDENING_SIGNING_EVIDENCE"] = signing_evidence
if signing_report:
    env["APOLYSIS_PRODUCTION_HARDENING_SIGNING_REPORT"] = signing_report

regulated_release_to_production_hardening_env = {
    "APOLYSIS_REGULATED_RELEASE_AWS_REGION": "APOLYSIS_PRODUCTION_HARDENING_AWS_REGION",
    "APOLYSIS_REGULATED_RELEASE_AWS_KMS_KEY_ID": "APOLYSIS_PRODUCTION_HARDENING_AWS_KMS_KEY_ID",
    "APOLYSIS_REGULATED_RELEASE_AWS_ROLE_TO_ASSUME": "APOLYSIS_PRODUCTION_HARDENING_AWS_ROLE_TO_ASSUME",
    "APOLYSIS_REGULATED_RELEASE_EXTERNAL_HSM_PKCS11_MODULE": "APOLYSIS_PRODUCTION_HARDENING_EXTERNAL_HSM_PKCS11_MODULE",
    "APOLYSIS_REGULATED_RELEASE_EXTERNAL_HSM_TOKEN_LABEL": "APOLYSIS_PRODUCTION_HARDENING_EXTERNAL_HSM_TOKEN_LABEL",
    "APOLYSIS_REGULATED_RELEASE_EXTERNAL_HSM_SLOT": "APOLYSIS_PRODUCTION_HARDENING_EXTERNAL_HSM_SLOT",
    "APOLYSIS_REGULATED_RELEASE_EXTERNAL_HSM_KEY_LABEL": "APOLYSIS_PRODUCTION_HARDENING_EXTERNAL_HSM_KEY_LABEL",
    "APOLYSIS_REGULATED_RELEASE_EXTERNAL_HSM_PIN_FILE": "APOLYSIS_PRODUCTION_HARDENING_EXTERNAL_HSM_PIN_FILE",
    "APOLYSIS_REGULATED_RELEASE_EXTERNAL_HSM_PIN": "APOLYSIS_PRODUCTION_HARDENING_EXTERNAL_HSM_PIN",
    "APOLYSIS_REGULATED_RELEASE_EXTERNAL_HSM_ALLOW_INTERACTIVE_PIN": "APOLYSIS_PRODUCTION_HARDENING_EXTERNAL_HSM_ALLOW_INTERACTIVE_PIN",
}
for source, target in regulated_release_to_production_hardening_env.items():
    if os.environ.get(source, ""):
        env[target] = os.environ[source]

if os.environ.get("APOLYSIS_REGULATED_RELEASE_AWS_KMS_ALIAS", ""):
    env["APOLYSIS_PRODUCTION_HARDENING_AWS_KMS_ALIAS"] = os.environ["APOLYSIS_REGULATED_RELEASE_AWS_KMS_ALIAS"]
    env.setdefault("APOLYSIS_PRODUCTION_HARDENING_AWS_KMS_KEY_ID", os.environ["APOLYSIS_REGULATED_RELEASE_AWS_KMS_ALIAS"])
...
downstream_output_path = output_dir / "production-hardening-signing-provider-readiness.out"
...
if process.returncode != 0:
    missing_requirements.append("production_hardening_signing_provider_readiness_audit_succeeded")
...
report = {
    "schema_version": 1,
    "phase": "regulated-release.signing-evidence",
    ...
    "execution_prerequisites": {
        ...
        "aws_kms": {
            ...
            "kms_key_id_or_alias_present": bool(
                aws_doc.get("APOLYSIS_PRODUCTION_HARDENING_AWS_KMS_KEY_ID")
                or os.environ.get("APOLYSIS_REGULATED_RELEASE_AWS_KMS_ALIAS", "")
            ),
        ...
        "external_hsm": {
            ...
            "module_present": bool(external_doc.get("APOLYSIS_PRODUCTION_HARDENING_EXTERNAL_HSM_PKCS11_MODULE")),
            "token_or_slot_present": bool(external_doc.get("APOLYSIS_PRODUCTION_HARDENING_EXTERNAL_HSM_TOKEN_LABEL_OR_SLOT")),
            "key_label_present": bool(external_doc.get("APOLYSIS_PRODUCTION_HARDENING_EXTERNAL_HSM_KEY_LABEL")),
    ...
    "notes": [
        ...
        "regulated-release.signing-evidence maps RegulatedRelease signing evidence controls to the historical production-hardening.signing-provider-readiness signing-provider readiness gate.",
    ...
    "next_commands": {
        "audit": "./scripts/test-regulated-release-signing-evidence.sh",
        "audit_retained_signing": (
            "APOLYSIS_REGULATED_RELEASE_SIGNING_EVIDENCE=<signing-evidence.json> "
            "APOLYSIS_REGULATED_RELEASE_SIGNING_REPORT=<signing-report.json> "
            "APOLYSIS_REQUIRE_REGULATED_RELEASE_SIGNING_EVIDENCE=1 ./scripts/test-regulated-release-signing-evidence.sh"
        ),
        "aws_kms_evidence": (
            "APOLYSIS_CONFIRM_PRODUCTION_HARDENING_AWS_KMS_SIGNING=1 "
            "APOLYSIS_PRODUCTION_HARDENING_AWS_KMS_KEY_ID=<key-id-or-arn> ./scripts/test-production-hardening-aws-kms-signing.sh"
        ),
        "external_hsm_evidence": (
            "APOLYSIS_CONFIRM_PRODUCTION_HARDENING_EXTERNAL_HSM_SIGNING=1 "
            ...
            "APOLYSIS_PRODUCTION_HARDENING_EXTERNAL_HSM_KEY_LABEL=<key> ./scripts/test-production-hardening-external-hsm-signing.sh"
        ),
    },
...
if require_ready and not signing_evidence_ready:
    print(f"apolysis-regulated_release: signing evidence failed closed ({report_path})", file=sys.stderr)
    print("missing requirements: " + ", ".join(report["missing_requirements"]), file=sys.stderr)
    raise SystemExit(1)
PY

cat <<EOF
apolysis-regulated_release: signing evidence audit written ($output_dir)
APOLYSIS_REGULATED_RELEASE_SIGNING_EVIDENCE_REPORT=$report
EOF
