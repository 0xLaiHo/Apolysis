...
output_dir="${APOLYSIS_F5_WORM_ARCHIVE_EXECUTION_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/f5-worm-archive-execution.XXXXXX")}"
mkdir -p "$output_dir"
output_dir="$(cd "$output_dir" && pwd)"
minio_image="${APOLYSIS_F5_MINIO_IMAGE:-minio/minio:RELEASE.2024-12-18T13-15-44Z}"
minio_name="${APOLYSIS_F5_MINIO_NAME:-apolysis-f5-worm-$stamp}"
access_key="${APOLYSIS_F5_MINIO_ACCESS_KEY:-apolysisf5}"
secret_key="${APOLYSIS_F5_MINIO_SECRET_KEY:-apolysisf5secret}"
bucket="${APOLYSIS_F5_WORM_BUCKET:-apolysis-f5-worm-$stamp}"
object_key="${APOLYSIS_F5_WORM_OBJECT_KEY:-releases/apolysis/$stamp/release-manifest.json}"
retention_days="${APOLYSIS_F5_WORM_RETENTION_DAYS:-365}"
evidence="$output_dir/apolysis-f5-worm-archive-execution-evidence.json"
report="$output_dir/apolysis-f5-worm-archive-execution-report.json"
fail_evidence="$output_dir/apolysis-f5-worm-archive-execution-evidence-fail.json"
fail_report="$output_dir/apolysis-f5-worm-archive-execution-report-fail.json"
...
if ! docker image inspect "$minio_image" >/dev/null 2>&1; then
    echo "apolysis-f5: missing MinIO image: $minio_image" >&2
    echo "apolysis-f5: pull it first or set APOLYSIS_F5_MINIO_IMAGE to an available image" >&2
    exit 1
fi

minio_port="${APOLYSIS_F5_MINIO_PORT:-$(choose_free_port)}"
...
if ! curl -fsS "$endpoint/minio/health/ready" >/dev/null 2>&1; then
    echo "apolysis-f5: MinIO did not become ready" >&2
...
manifest = {
    "schema": "apolysis.dev/f5-worm-archive-execution-manifest/v1",
...
release_manifest = output_dir / "apolysis-f5-worm-release-manifest.json"
...
observations_path = output_dir / "apolysis-f5-worm-s3-api-observations.json"
observations_path.write_text(json.dumps(observations, indent=2, sort_keys=True) + "\n")

evidence = {
    "evidence_id": "f5-worm-archive-execution",
...
cargo run -q -p apolysis-validation --bin apolysis-f5-worm-archive-execution-evidence -- \
...
if cargo run -q -p apolysis-validation --bin apolysis-f5-worm-archive-execution-evidence -- \
    --evidence "$fail_evidence" >"$fail_report"; then
    echo "apolysis-f5: invalid WORM archive execution evidence unexpectedly passed" >&2
    exit 1
fi
...
printf 'apolysis-f5: WORM archive API execution gate passed (%s)\n' "$output_dir"
