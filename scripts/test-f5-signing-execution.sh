...
output_dir="${APOLYSIS_F5_SIGNING_EXECUTION_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/f5-signing-execution.XXXXXX")}"
mkdir -p "$output_dir"
...
for command in cargo jq openssl pkcs11-tool python3 sha256sum softhsm2-util; do
    require_command "$command"
done

module="${APOLYSIS_F5_PKCS11_MODULE:-/usr/lib/pkcs11/libsofthsm2.so}"
if [[ ! -r "$module" ]]; then
    echo "apolysis-f5: missing readable PKCS#11 module: $module" >&2
    exit 1
fi

token_root="$output_dir/softhsm-token-store"
softhsm_conf="$output_dir/softhsm2.conf"
token_label="apolysis-f5-release"
key_label="f5-release"
key_id="01"
pin="123456"
so_pin="12345678"
...
signature="$output_dir/apolysis-f5-release-manifest.pkcs11.sig"
...
evidence="$output_dir/apolysis-f5-signing-execution-evidence.json"
report="$output_dir/apolysis-f5-signing-execution-report.json"
fail_evidence="$output_dir/apolysis-f5-signing-execution-evidence-fail.json"
fail_report="$output_dir/apolysis-f5-signing-execution-report-fail.json"
...
cat >"$release_manifest" <<'JSON'
{
  ...
  "signing": {
    "keyMode": "hsm",
    "publicKey": "apolysis-f5-release-public.pem",
    "manifestBundle": "apolysis-f5-release-manifest.pkcs11.sig",
    "provenanceBundle": "apolysis-f5-provenance.pkcs11.sig"
  }
}
JSON
...
grep -q 'Private Key Object; RSA  2048 bits' "$objects_log" || {
    echo "apolysis-f5: PKCS#11 private key was not generated" >&2
    exit 1
}
grep -q 'Access:     sensitive, always sensitive, never extractable, local' "$objects_log" || {
    echo "apolysis-f5: PKCS#11 private key is not marked sensitive and never extractable" >&2
    exit 1
}
...
if [[ -s "$output_dir/apolysis-f5-release-private.der" ]]; then
    echo "apolysis-f5: PKCS#11 private key was unexpectedly exportable" >&2
    exit 1
fi
...
grep -q 'Verified OK' "$verify_log" || {
    echo "apolysis-f5: OpenSSL did not verify the PKCS#11 signature" >&2
...
signer_tool="pkcs11-tool with $(softhsm2-util --version | tr '\n' ' ')"
verifier_tool="$(openssl version)"

cleanup_token_store
if [[ -e "$token_root" ]]; then
    echo "apolysis-f5: temporary SoftHSM token store was not removed" >&2
    exit 1
fi
...
evidence = {
    "evidence_id": "f5-pkcs11-signing-execution",
...
jq -e '
  ...
  and (.approval.key_uri | startswith("pkcs11:token=apolysis-f5-release;object=f5-release"))
' "$report" >/dev/null
...
if cargo run -q -p apolysis-validation --bin apolysis-f5-signing-execution-evidence -- \
    --evidence "$fail_evidence" >"$fail_report"; then
    echo "apolysis-f5: invalid signing execution evidence unexpectedly passed" >&2
    exit 1
fi
...
printf 'apolysis-f5: PKCS#11 signing execution gate passed (%s)\n' "$output_dir"
