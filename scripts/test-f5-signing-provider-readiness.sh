...
output_dir="${APOLYSIS_F5_SIGNING_PROVIDER_READINESS_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/f5-signing-provider-readiness.XXXXXX")}"
mkdir -p "$output_dir"
output_dir="$(cd "$output_dir" && pwd)"

report="$output_dir/apolysis-f5-signing-provider-readiness-report.json"
require_ready="${APOLYSIS_REQUIRE_F5_SIGNING_PROVIDER_READINESS:-0}"
...
signing_evidence_path = env_path("APOLYSIS_F5_SIGNING_EVIDENCE")
signing_report_path = env_path("APOLYSIS_F5_SIGNING_REPORT")
...
aws_kms = {
    ...
    "APOLYSIS_F5_AWS_KMS_KEY_ID": bool(os.environ.get("APOLYSIS_F5_AWS_KMS_KEY_ID")),
    "region": bool(aws_region),
    "credential_hint_present": bool(aws_credential_hint),
    "confirmation_required": "APOLYSIS_CONFIRM_F5_AWS_KMS_SIGNING=1",
}
...
external_hsm_module = os.environ.get("APOLYSIS_F5_EXTERNAL_HSM_PKCS11_MODULE", "")
...
external_hsm_pin_configured = bool(os.environ.get("APOLYSIS_F5_EXTERNAL_HSM_PIN_FILE")) or bool(
    os.environ.get("APOLYSIS_F5_EXTERNAL_HSM_PIN")
)
external_hsm_interactive_pin = os.environ.get("APOLYSIS_F5_EXTERNAL_HSM_ALLOW_INTERACTIVE_PIN", "0") == "1"
slot_list_ok, slot_list_output = list_slots(external_hsm_module_realpath)
token_label = os.environ.get("APOLYSIS_F5_EXTERNAL_HSM_TOKEN_LABEL", "")
slot = os.environ.get("APOLYSIS_F5_EXTERNAL_HSM_SLOT", "")
...
external_hsm = {
    ...
    "APOLYSIS_F5_EXTERNAL_HSM_PKCS11_MODULE": bool(external_hsm_module),
    ...
    "APOLYSIS_F5_EXTERNAL_HSM_TOKEN_LABEL_or_SLOT": token_or_slot_configured,
    "APOLYSIS_F5_EXTERNAL_HSM_KEY_LABEL": bool(os.environ.get("APOLYSIS_F5_EXTERNAL_HSM_KEY_LABEL")),
    ...
    "confirmation_required": "APOLYSIS_CONFIRM_F5_EXTERNAL_HSM_SIGNING=1",
}
...
report = {
    ...
    "notes": [
        ...
        "A ready AWS KMS path still requires running scripts/test-f5-aws-kms-signing.sh to create retained signing evidence.",
        "A ready external HSM path still requires running scripts/test-f5-external-hsm-signing.sh to create retained signing evidence.",
        "SoftHSM and local TPM evidence do not satisfy the current F5 cloud KMS/external hardware HSM requirement.",
    ],
    "next_commands": {
        "aws_kms": "APOLYSIS_CONFIRM_F5_AWS_KMS_SIGNING=1 APOLYSIS_F5_AWS_KMS_KEY_ID=<key-id-or-arn> ./scripts/test-f5-aws-kms-signing.sh",
        "external_hsm": "APOLYSIS_CONFIRM_F5_EXTERNAL_HSM_SIGNING=1 APOLYSIS_F5_EXTERNAL_HSM_PKCS11_MODULE=<module> APOLYSIS_F5_EXTERNAL_HSM_TOKEN_LABEL=<token> APOLYSIS_F5_EXTERNAL_HSM_KEY_LABEL=<key> ./scripts/test-f5-external-hsm-signing.sh",
    },
...
if require_ready and not signing_provider_ready:
    print(f"apolysis-f5: signing provider readiness failed closed ({report_path})", file=sys.stderr)
    print("missing requirements: " + ", ".join(missing), file=sys.stderr)
    raise SystemExit(1)
PY

cat <<EOF
apolysis-f5: signing provider readiness audit written ($output_dir)
APOLYSIS_F5_SIGNING_PROVIDER_READINESS_REPORT=$report
EOF
