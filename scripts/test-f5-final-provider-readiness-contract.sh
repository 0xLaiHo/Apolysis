...
output_dir="${APOLYSIS_F5_FINAL_PROVIDER_READINESS_CONTRACT_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/f5-final-provider-readiness-contract.XXXXXX")}"
mkdir -p "$output_dir"
...
APOLYSIS_F5_FINAL_PROVIDER_READINESS_OUTPUT_DIR="$run_dir" \
...
if [[ "$readiness_rc" -eq 0 ]]; then
    echo "apolysis-f5: final provider readiness accepted fixture artifacts without live_provider evidence source" >&2
    cat "$output_dir/readiness.out" >&2
    cat "$output_dir/readiness.err" >&2
    exit 1
fi

report="$run_dir/apolysis-f5-final-provider-readiness-report.json"
jq -e '
  ...
  and .artifact_inputs.APOLYSIS_F5_SIGNING_EVIDENCE.live_provider_evidence == false
  and .artifact_inputs.APOLYSIS_F5_MANAGED_MESH_EVIDENCE.live_provider_evidence == false
' "$report" >/dev/null

printf 'apolysis-f5: final provider readiness fixture rejection contract passed (%s)\n' "$output_dir"
