...
confirm="${APOLYSIS_CONFIRM_F5_AWS_KMS_SIGNING:-0}"

if [[ "$confirm" != "1" ]]; then
    cat >&2 <<'EOF'
apolysis-f5: AWS KMS live signing is opt-in.
Set APOLYSIS_CONFIRM_F5_AWS_KMS_SIGNING=1 and APOLYSIS_F5_AWS_KMS_KEY_ID
...
if [[ -z "$key_id" ]]; then
    echo "apolysis-f5: APOLYSIS_F5_AWS_KMS_KEY_ID is required" >&2
    exit 2
fi
...
if [[ "$algorithm" != "RSASSA_PKCS1_V1_5_SHA_256" ]]; then
    echo "apolysis-f5: only RSASSA_PKCS1_V1_5_SHA_256 is currently supported" >&2
    exit 2
fi

mkdir -p "$repo_root/target"
output_dir="${APOLYSIS_F5_AWS_KMS_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/f5-aws-kms-signing.XXXXXX")}"
mkdir -p "$output_dir"
output_dir="$(cd "$output_dir" && pwd)"

release_manifest="$output_dir/apolysis-f5-release-manifest.json"
release_manifest_digest_bin="$output_dir/apolysis-f5-release-manifest.sha256.bin"
...
signature="$output_dir/apolysis-f5-release-manifest.aws-kms.sig"
verify_log="$output_dir/openssl-verify.out"
evidence="$output_dir/apolysis-f5-aws-kms-signing-evidence.json"
report="$output_dir/apolysis-f5-aws-kms-signing-report.json"
...
cat >"$release_manifest" <<'JSON'
{
  ...
  "signing": {
    "keyMode": "cloud_kms",
    "provider": "aws_kms",
    "manifestBundle": "apolysis-f5-release-manifest.aws-kms.sig"
  }
}
JSON
...
' "$key_metadata" >/dev/null || {
    echo "apolysis-f5: AWS KMS key must be enabled, provider-generated, and usable for RSA SHA-256 signing" >&2
    exit 1
}
...
grep -q 'Verified OK' "$verify_log" || {
    echo "apolysis-f5: OpenSSL did not verify the AWS KMS signature" >&2
...
evidence = {
    "evidence_id": f"f5-aws-kms-signing-{observed_at_unix_ms}",
...
printf 'apolysis-f5: AWS KMS signing gate passed (%s)\n' "$output_dir"
