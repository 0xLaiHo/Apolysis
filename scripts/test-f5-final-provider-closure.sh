...
output_dir="${APOLYSIS_F5_FINAL_PROVIDER_CLOSURE_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/f5-final-provider-closure.XXXXXX")}"
mkdir -p "$output_dir"
output_dir="$(cd "$output_dir" && pwd)"

report="$output_dir/apolysis-f5-final-provider-closure-report.json"
require_ready="${APOLYSIS_REQUIRE_F5_FINAL_PROVIDER_CLOSURE:-0}"
run_completion="${APOLYSIS_RUN_F5_FINAL_PROVIDER_COMPLETION:-0}"
...
def load_env_file(path: Path | None) -> dict[str, str]:
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        if name.startswith("APOLYSIS_F5_"):
            exports[name] = value
    return exports

readiness_step = run_step(
    "provider-workflow-readiness",
    [str(repo_root / "scripts/test-f5-provider-workflow-readiness.sh")],
    {
        "APOLYSIS_F5_PROVIDER_WORKFLOW_READINESS_OUTPUT_DIR": str(output_dir / "provider-workflow-readiness"),
        "APOLYSIS_REQUIRE_F5_PROVIDER_WORKFLOW_READINESS": "0",
    },
    "apolysis-f5-provider-workflow-readiness-report.json",
)
readiness_doc = load_json(readiness_step["report_file"])

dispatch_step = run_step(
    "provider-workflow-dispatch",
    [str(repo_root / "scripts/test-f5-provider-workflow-dispatch.sh")],
    {
        "APOLYSIS_F5_PROVIDER_WORKFLOW_DISPATCH_OUTPUT_DIR": str(output_dir / "provider-workflow-dispatch"),
        "APOLYSIS_REQUIRE_F5_PROVIDER_WORKFLOW_DISPATCH": "0",
    },
    "apolysis-f5-provider-workflow-dispatch-report.json",
)
dispatch_doc = load_json(dispatch_step["report_file"])

artifact_import_step = run_step(
    "provider-workflow-artifact-import",
    [str(repo_root / "scripts/test-f5-provider-workflow-artifact-import.sh")],
    {
        "APOLYSIS_F5_PROVIDER_WORKFLOW_ARTIFACT_IMPORT_OUTPUT_DIR": str(output_dir / "provider-workflow-artifact-import"),
        "APOLYSIS_REQUIRE_F5_PROVIDER_WORKFLOW_ARTIFACT_IMPORT": "0",
    },
    "apolysis-f5-provider-workflow-artifact-import-report.json",
)
...
if run_completion:
    completion_step = run_step(
        ...
        [str(repo_root / "scripts/verify-f5-final-provider-completion.sh")],
        {
            **completion_env_exports,
            "APOLYSIS_F5_FINAL_PROVIDER_COMPLETION_OUTPUT_DIR": str(output_dir / "final-provider-completion"),
        },
        "apolysis-f5-final-provider-completion-report.json",
    )
...
if require_ready and not run_completion:
    missing_requirements.append("APOLYSIS_RUN_F5_FINAL_PROVIDER_COMPLETION")
...
report = {
    ...
    "notes": [
        ...
        "Set APOLYSIS_RUN_F5_FINAL_PROVIDER_COMPLETION=1 to run scripts/verify-f5-final-provider-completion.sh with provider artifact exports from the F5.46/F5.31 env file.",
    ...
    "next_commands": {
        "audit_closure": "./scripts/test-f5-final-provider-closure.sh",
        "required_completion_from_local_artifacts": "APOLYSIS_F5_PROVIDER_WORKFLOW_ARTIFACT_ROOT=<artifact-root> APOLYSIS_RUN_F5_FINAL_PROVIDER_COMPLETION=1 APOLYSIS_REQUIRE_F5_FINAL_PROVIDER_CLOSURE=1 ./scripts/test-f5-final-provider-closure.sh",
        "download_then_complete": "APOLYSIS_F5_PROVIDER_WORKFLOW_ARTIFACT_IMPORT_MODE=download APOLYSIS_CONFIRM_F5_PROVIDER_WORKFLOW_ARTIFACT_DOWNLOAD=1 APOLYSIS_F5_PROVIDER_WORKFLOW_RUN_ID=<run-id> APOLYSIS_RUN_F5_FINAL_PROVIDER_COMPLETION=1 APOLYSIS_REQUIRE_F5_FINAL_PROVIDER_CLOSURE=1 ./scripts/test-f5-final-provider-closure.sh",
    },
...
if require_ready and not closure_ready:
    print(f"apolysis-f5: final provider closure failed closed ({report_path})", file=sys.stderr)
    print("missing requirements: " + ", ".join(missing_requirements), file=sys.stderr)
    raise SystemExit(1)
PY

cat <<EOF
apolysis-f5: final provider closure audit written ($output_dir)
APOLYSIS_F5_FINAL_PROVIDER_CLOSURE_REPORT=$report
EOF
