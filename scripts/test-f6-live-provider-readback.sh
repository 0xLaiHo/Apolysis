...
output_dir="${APOLYSIS_F6_LIVE_PROVIDER_READBACK_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/f6-live-provider-readback.XXXXXX")}"
mkdir -p "$output_dir"
output_dir="$(cd "$output_dir" && pwd)"

report="$output_dir/apolysis-f6-live-provider-readback-report.json"
require_ready="${APOLYSIS_REQUIRE_F6_LIVE_PROVIDER_READBACK:-0}"
...
manifest_path = output_dir / "apolysis-f6-live-provider-readback-manifest.json"
...
external_report_value = env_value("APOLYSIS_F6_EXTERNAL_RETENTION_REPORT")
registry_report_value = env_value(
    "APOLYSIS_F6_IMMUTABLE_REGISTRY_RETENTION_REPORT",
    "APOLYSIS_F6_IMMUTABLE_REGISTRY_REPORT",
)
external_readback_value = env_value("APOLYSIS_F6_EXTERNAL_RETENTION_READBACK_EVIDENCE")
registry_readback_value = env_value("APOLYSIS_F6_IMMUTABLE_REGISTRY_READBACK_EVIDENCE")
...
if external_report_path is None:
    missing_requirements.append("APOLYSIS_F6_EXTERNAL_RETENTION_REPORT")
...
if registry_report_path is None:
    missing_requirements.append("APOLYSIS_F6_IMMUTABLE_REGISTRY_RETENTION_REPORT")
...
if external_readback_path is None:
    missing_requirements.append("APOLYSIS_F6_EXTERNAL_RETENTION_READBACK_EVIDENCE")
...
if registry_readback_path is None:
    missing_requirements.append("APOLYSIS_F6_IMMUTABLE_REGISTRY_READBACK_EVIDENCE")
...
external_policy_verified = bool_env("APOLYSIS_F6_EXTERNAL_READBACK_RETENTION_VERIFIED")
...
external_delete_denied = bool_env("APOLYSIS_F6_EXTERNAL_READBACK_DELETE_DENIED")
...
registry_policy_verified = bool_env("APOLYSIS_F6_REGISTRY_READBACK_IMMUTABILITY_VERIFIED")
...
registry_mutation_denied = bool_env("APOLYSIS_F6_REGISTRY_READBACK_MUTATION_DENIED")
...
manifest = {
    "schema_version": 1,
    "phase": "F6.11",
    "source": "f6_live_provider_readback",
    "external_retention": {
...
report = {
    ...
    "notes": [
        ...
        "The F6.11 gate validates retained provider-side readback evidence for external retention and immutable registry controls.",
        ...
        "Required mode fails closed unless both external retention and immutable registry readback evidence match the F6 reports.",
    ],
    "next_commands": {
        "audit": "./scripts/test-f6-live-provider-readback.sh",
        "required_metadata": "APOLYSIS_F6_EXTERNAL_RETENTION_REPORT=<external-retention-report> APOLYSIS_F6_IMMUTABLE_REGISTRY_RETENTION_REPORT=<immutable-registry-report> APOLYSIS_F6_EXTERNAL_RETENTION_READBACK_EVIDENCE=<external-readback.json> APOLYSIS_F6_IMMUTABLE_REGISTRY_READBACK_EVIDENCE=<registry-readback.json> APOLYSIS_REQUIRE_F6_LIVE_PROVIDER_READBACK=1 ./scripts/test-f6-live-provider-readback.sh",
    },
...
if require_ready and not live_provider_readback_ready:
    print(f"apolysis-f6: live provider readback failed closed ({report_path})", file=sys.stderr)
    print("missing requirements: " + ", ".join(report["missing_requirements"]), file=sys.stderr)
    raise SystemExit(1)
PY

cat <<EOF
apolysis-f6: live provider readback audit written ($output_dir)
APOLYSIS_F6_LIVE_PROVIDER_READBACK_REPORT=$report
EOF
