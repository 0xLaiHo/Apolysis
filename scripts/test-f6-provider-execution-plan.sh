...
output_dir="${APOLYSIS_F6_PROVIDER_EXECUTION_PLAN_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/f6-provider-execution-plan.XXXXXX")}"
mkdir -p "$output_dir"
output_dir="$(cd "$output_dir" && pwd)"

report="$output_dir/apolysis-f6-provider-execution-plan-report.json"
require_ready="${APOLYSIS_REQUIRE_F6_PROVIDER_EXECUTION_PLAN:-0}"
...
signing_provider = os.environ.get("APOLYSIS_F6_SIGNING_PROVIDER", "auto")
artifact_source = (
    os.environ.get("APOLYSIS_F6_ARTIFACT_SOURCE")
    or os.environ.get("APOLYSIS_F6_PROVIDER_ARTIFACT_SOURCE")
    or "auto"
)
closure_mode = os.environ.get("APOLYSIS_F6_CLOSURE_MODE", "audit")
...
if signing_provider not in allowed_signing:
    errors.append("APOLYSIS_F6_SIGNING_PROVIDER")
if artifact_source not in allowed_artifacts:
    errors.append("APOLYSIS_F6_ARTIFACT_SOURCE")
if closure_mode not in allowed_closure:
    errors.append("APOLYSIS_F6_CLOSURE_MODE")
...
aws_kms_key = bool_env("APOLYSIS_F5_AWS_KMS_KEY_ID") or bool_env("APOLYSIS_F5_AWS_KMS_ALIAS")
aws_role_or_credential_hint = (
    bool_env("APOLYSIS_F5_AWS_ROLE_TO_ASSUME")
...
external_hsm_ready = (
    ...
    and bool_env("APOLYSIS_F5_EXTERNAL_HSM_PKCS11_MODULE")
    and (bool_env("APOLYSIS_F5_EXTERNAL_HSM_TOKEN_LABEL") or bool_env("APOLYSIS_F5_EXTERNAL_HSM_SLOT"))
    and bool_env("APOLYSIS_F5_EXTERNAL_HSM_KEY_LABEL")
    and (
        bool_env("APOLYSIS_F5_EXTERNAL_HSM_PIN_FILE")
        or bool_env("APOLYSIS_F5_EXTERNAL_HSM_PIN")
...
local_artifact_root = os.environ.get("APOLYSIS_F6_PROVIDER_ARTIFACT_ROOT") or os.environ.get(
    "APOLYSIS_F5_PROVIDER_WORKFLOW_ARTIFACT_ROOT", ""
)
workflow_run_id = os.environ.get("APOLYSIS_F6_PROVIDER_WORKFLOW_RUN_ID") or os.environ.get(
    "APOLYSIS_F5_PROVIDER_WORKFLOW_RUN_ID", ""
)
retained_package = os.environ.get("APOLYSIS_F6_RETAINED_PROVIDER_ARTIFACT_PACKAGE") or os.environ.get(
    "APOLYSIS_F5_RETAINED_PROVIDER_ARTIFACT_PACKAGE", ""
)
retained_package_sha256 = os.environ.get("APOLYSIS_F6_RETAINED_PROVIDER_ARTIFACT_PACKAGE_SHA256") or os.environ.get(
...
closure_plan_ready = closure_mode == "final_provider_closure" or os.environ.get(
    "APOLYSIS_RUN_F6_FINAL_PROVIDER_CLOSURE", "0"
) == "1"
...
command_templates = {
    "aws_kms_signing": (
        "APOLYSIS_F6_SIGNING_PROVIDER=aws_kms APOLYSIS_F5_AWS_REGION=<region> "
        "APOLYSIS_F5_AWS_KMS_KEY_ID=<key-or-arn> APOLYSIS_F5_AWS_ROLE_TO_ASSUME=<role-arn> "
        "./scripts/test-f6-provider-execution-plan.sh"
    ),
    "external_hsm_signing": (
        "APOLYSIS_F6_SIGNING_PROVIDER=external_hsm "
        "APOLYSIS_F5_EXTERNAL_HSM_PKCS11_MODULE=<module> "
        "APOLYSIS_F5_EXTERNAL_HSM_TOKEN_LABEL=<token> "
        "APOLYSIS_F5_EXTERNAL_HSM_KEY_LABEL=<key> "
        "APOLYSIS_F5_EXTERNAL_HSM_PIN_FILE=<pin-file> ./scripts/test-f6-provider-execution-plan.sh"
    ),
    "local_artifact_root": (
        "APOLYSIS_F6_ARTIFACT_SOURCE=local_artifact_root "
        "APOLYSIS_F6_PROVIDER_ARTIFACT_ROOT=<artifact-root> "
        "./scripts/test-f6-provider-execution-plan.sh"
    ),
    "workflow_download": (
        "APOLYSIS_F6_ARTIFACT_SOURCE=workflow_download "
        "APOLYSIS_F6_PROVIDER_WORKFLOW_RUN_ID=<run-id> "
        "APOLYSIS_CONFIRM_F6_PROVIDER_ARTIFACT_DOWNLOAD=1 ./scripts/test-f6-provider-execution-plan.sh"
    ),
    "retained_package": (
        "APOLYSIS_F6_ARTIFACT_SOURCE=retained_package "
    ...
    "execute_regulated_release": (
        "APOLYSIS_RUN_F6_FINAL_PROVIDER_CLOSURE=1 "
        "APOLYSIS_REQUIRE_F6_REGULATED_RELEASE=1 ./scripts/test-f6-regulated-release.sh"
    ),
}
...
report = {
    ...
    "signing_provider_plan": {
        ...
        "external_hsm": {
            ...
            "module_present": bool_env("APOLYSIS_F5_EXTERNAL_HSM_PKCS11_MODULE"),
            "token_or_slot_present": bool_env("APOLYSIS_F5_EXTERNAL_HSM_TOKEN_LABEL")
            or bool_env("APOLYSIS_F5_EXTERNAL_HSM_SLOT"),
            "key_label_present": bool_env("APOLYSIS_F5_EXTERNAL_HSM_KEY_LABEL"),
            "pin_source_or_interactive_present": bool_env("APOLYSIS_F5_EXTERNAL_HSM_PIN_FILE")
            or bool_env("APOLYSIS_F5_EXTERNAL_HSM_PIN")
            or os.environ.get("APOLYSIS_F5_EXTERNAL_HSM_ALLOW_INTERACTIVE_PIN", "0") == "1",
        },
        "retained_signing": {
    ...
    "artifact_source_plan": {
        ...
        "workflow_download": {
            ...
            "download_confirmation_required": "APOLYSIS_CONFIRM_F6_PROVIDER_ARTIFACT_DOWNLOAD=1",
        },
        "retained_package": {
    ...
    "closure_plan": {
        ...
        "requires": "APOLYSIS_RUN_F6_FINAL_PROVIDER_CLOSURE=1",
    },
    ...
    "notes": [
        ...
        "This gate creates a F6-native execution plan over historical F5 provider artifact contracts.",
...
if require_ready and not provider_execution_plan_ready:
    print(f"apolysis-f6: provider execution plan failed closed ({report_path})", file=sys.stderr)
    print("missing requirements: " + ", ".join(report["missing_requirements"]), file=sys.stderr)
    raise SystemExit(1)
PY

cat <<EOF
apolysis-f6: provider execution plan audit written ($output_dir)
APOLYSIS_F6_PROVIDER_EXECUTION_PLAN_REPORT=$report
EOF
