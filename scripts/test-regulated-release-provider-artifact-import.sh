...
output_dir="${APOLYSIS_REGULATED_RELEASE_PROVIDER_ARTIFACT_IMPORT_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/regulated-release-provider-artifact-import.XXXXXX")}"
mkdir -p "$output_dir"
output_dir="$(cd "$output_dir" && pwd)"

report="$output_dir/apolysis-regulated-release-provider-artifact-import-report.json"
require_ready="${APOLYSIS_REQUIRE_REGULATED_RELEASE_PROVIDER_ARTIFACT_IMPORT:-0}"
...
production_hardening_gate = repo_root / "scripts/test-production-hardening-provider-workflow-artifact-import.sh"
downstream_dir = output_dir / "production-hardening-provider-workflow-artifact-import"
downstream_report_path = downstream_dir / "apolysis-production-hardening-provider-workflow-artifact-import-report.json"
...
requested_source = env_value("APOLYSIS_REGULATED_RELEASE_PROVIDER_ARTIFACT_SOURCE", "APOLYSIS_REGULATED_RELEASE_ARTIFACT_SOURCE") or "auto"
...
env = os.environ.copy()
env["APOLYSIS_PRODUCTION_HARDENING_PROVIDER_WORKFLOW_ARTIFACT_IMPORT_OUTPUT_DIR"] = str(downstream_dir)
env["APOLYSIS_REQUIRE_PRODUCTION_HARDENING_PROVIDER_WORKFLOW_ARTIFACT_IMPORT"] = "0"
...
downstream_output_path = output_dir / "production-hardening-provider-workflow-artifact-import.out"
...
if process.returncode != 0:
    missing_requirements.append("production_hardening_provider_workflow_artifact_import_audit_succeeded")
...
report = {
    "schema_version": 1,
    "phase": "regulated-release.provider-artifact-import",
    ...
    "notes": [
        ...
        "regulated-release.provider-artifact-import maps RegulatedRelease provider artifact source controls to historical production-hardening.provider-workflow-artifact-import import contracts.",
        "The downstream gate is scripts/test-production-hardening-provider-workflow-artifact-import.sh.",
        "Default audit mode does not download workflow artifacts unless APOLYSIS_CONFIRM_REGULATED_RELEASE_PROVIDER_ARTIFACT_DOWNLOAD=1 is set.",
    ...
    "next_commands": {
        "audit": "./scripts/test-regulated-release-provider-artifact-import.sh",
        "audit_local_artifacts": (
            ...
            "APOLYSIS_REQUIRE_REGULATED_RELEASE_PROVIDER_ARTIFACT_IMPORT=1 ./scripts/test-regulated-release-provider-artifact-import.sh"
        ),
        ...
        "audit_retained_package": (
            "APOLYSIS_REGULATED_RELEASE_PROVIDER_ARTIFACT_SOURCE=retained_package "
...
if require_ready and not provider_artifact_import_ready:
    print(f"apolysis-regulated_release: provider artifact import failed closed ({report_path})", file=sys.stderr)
    print("missing requirements: " + ", ".join(report["missing_requirements"]), file=sys.stderr)
    raise SystemExit(1)
PY

cat <<EOF
apolysis-regulated_release: provider artifact import audit written ($output_dir)
APOLYSIS_REGULATED_RELEASE_PROVIDER_ARTIFACT_IMPORT_REPORT=$report
EOF
