...
output_dir="${APOLYSIS_F5_FINAL_PROVIDER_BUNDLE_ENV_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/f5-final-provider-bundle-env.XXXXXX")}"
mkdir -p "$output_dir"
output_dir="$(cd "$output_dir" && pwd)"

report="$output_dir/apolysis-f5-final-provider-bundle-env-report.json"
env_file="$output_dir/apolysis-f5-final-provider-bundle.env"
...
artifact_root_value = os.environ.get("APOLYSIS_F5_PROVIDER_ARTIFACT_ROOT", "")
explicit_artifact_roots = []
if artifact_root_value:
    explicit_artifact_roots.extend(Path(part) for part in artifact_root_value.split(":") if part)

require_ready = os.environ.get("APOLYSIS_REQUIRE_F5_FINAL_BUNDLE_ENV", "0") == "1"
run_final_bundle = os.environ.get("APOLYSIS_RUN_F5_FINAL_BUNDLE", "0") == "1"

known_defaults = {
    "worm_evidence": repo_root / "target/f5-cloudflare-r2-worm/apolysis-f5-cloudflare-r2-worm-evidence.json",
    "worm_report": repo_root / "target/f5-cloudflare-r2-worm/apolysis-f5-cloudflare-r2-worm-report.json",
    "registry_evidence": repo_root / "target/f5-dockerhub-registry-promotion.aByXvA/apolysis-f5-dockerhub-registry-promotion-evidence.json",
    "registry_report": repo_root / "target/f5-dockerhub-registry-promotion.aByXvA/apolysis-f5-dockerhub-registry-promotion-report.json",
}
...
env_names = {
    "signing_evidence": "APOLYSIS_F5_SIGNING_EVIDENCE",
    "signing_report": "APOLYSIS_F5_SIGNING_REPORT",
    "worm_evidence": "APOLYSIS_F5_WORM_EVIDENCE",
    "worm_report": "APOLYSIS_F5_WORM_REPORT",
    "registry_evidence": "APOLYSIS_F5_REGISTRY_EVIDENCE",
    "registry_report": "APOLYSIS_F5_REGISTRY_REPORT",
    "managed_mesh_evidence": "APOLYSIS_F5_MANAGED_MESH_EVIDENCE",
    "managed_mesh_report": "APOLYSIS_F5_MANAGED_MESH_REPORT",
}
...
if managed_mesh_provider:
    exports["APOLYSIS_F5_MANAGED_MESH_PROVIDER"] = managed_mesh_provider
if managed_mesh_control_plane:
    exports["APOLYSIS_F5_MANAGED_MESH_CONTROL_PLANE"] = managed_mesh_control_plane
if signing_provider:
    exports["APOLYSIS_F5_SIGNING_PROVIDER"] = signing_provider
if signing_control_plane:
    exports["APOLYSIS_F5_SIGNING_CONTROL_PLANE"] = signing_control_plane

env_lines = [
    "# Source this file before running scripts/build-f5-final-external-provider-bundle.sh.",
]
...
if run_final_bundle:
    if not all_ready:
        ...
        process = subprocess.run(
            [str(repo_root / "scripts/build-f5-final-external-provider-bundle.sh")],
            cwd=repo_root,
            env=env,
            text=True,
...
if require_ready and not all_ready:
    print(f"apolysis-f5: final provider bundle env failed closed ({report_path})", file=sys.stderr)
    print("missing requirements: " + ", ".join(missing), file=sys.stderr)
    raise SystemExit(1)
if run_final_bundle and final_bundle_status != "passed":
    print(f"apolysis-f5: final bundle builder did not pass ({report_path})", file=sys.stderr)
    raise SystemExit(1)
PY

cat <<EOF
apolysis-f5: final provider bundle env audit written ($output_dir)
APOLYSIS_F5_FINAL_PROVIDER_BUNDLE_ENV=$env_file
APOLYSIS_F5_FINAL_PROVIDER_BUNDLE_ENV_REPORT=$report
EOF
