...
output_dir="${APOLYSIS_F6_MANAGED_MESH_DECISION_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/f6-managed-mesh-decision.XXXXXX")}"
mkdir -p "$output_dir"
output_dir="$(cd "$output_dir" && pwd)"

report="$output_dir/apolysis-f6-managed-mesh-decision-report.json"
require_ready="${APOLYSIS_REQUIRE_F6_MANAGED_MESH_DECISION:-0}"
...
manifest_path = output_dir / "apolysis-f6-managed-mesh-decision-manifest.json"
...
evidence_value = env_value("APOLYSIS_F6_MANAGED_MESH_EVIDENCE", "APOLYSIS_F5_MANAGED_MESH_EVIDENCE")
report_value = env_value("APOLYSIS_F6_MANAGED_MESH_REPORT", "APOLYSIS_F5_MANAGED_MESH_REPORT")
...
if evidence_path is None:
    missing_requirements.append("APOLYSIS_F6_MANAGED_MESH_EVIDENCE")
elif not evidence_path.is_file():
    missing_requirements.append("managed_mesh_evidence")

if mesh_report_path is None:
    missing_requirements.append("APOLYSIS_F6_MANAGED_MESH_REPORT")
elif not mesh_report_path.is_file():
    missing_requirements.append("managed_mesh_report")

provider = env_value("APOLYSIS_F6_MANAGED_MESH_PROVIDER", "APOLYSIS_F5_MANAGED_MESH_PROVIDER") or first_text_value(
...
provider_control_plane = env_value(
    ...
    "APOLYSIS_F5_MANAGED_MESH_CONTROL_PLANE",
) or first_text_value(evidence_doc, "provider_control_plane", "mesh_uri", "cluster_name")
decision = env_value("APOLYSIS_F6_MANAGED_MESH_DECISION") or first_text_value(
    evidence_doc, "managed_mesh_decision", "decision"
)
rationale = env_value("APOLYSIS_F6_MANAGED_MESH_DECISION_RATIONALE") or first_text_value(
...
additional_required_value = env_value("APOLYSIS_F6_MANAGED_MESH_ADDITIONAL_PROVIDER_REQUIRED")
...
manifest = {
    "schema_version": 1,
    "phase": "F6.10",
    "source": "f6_managed_mesh_decision",
    "provider": provider,
...
report = {
    ...
    "notes": [
        ...
        "The F6.10 gate records the regulated-release decision for managed mesh evidence.",
        "This gate does not call Kubernetes, service-mesh, or cloud-provider APIs.",
        "Vultr VKE Istio evidence is accepted only when the operator records an explicit F6 decision.",
    ],
    "next_commands": {
        "audit": "./scripts/test-f6-managed-mesh-decision.sh",
        "required_metadata": "APOLYSIS_F6_MANAGED_MESH_EVIDENCE=<managed-mesh-evidence.json> APOLYSIS_F6_MANAGED_MESH_REPORT=<managed-mesh-report.json> APOLYSIS_F6_MANAGED_MESH_DECISION=<decision> APOLYSIS_F6_MANAGED_MESH_DECISION_RATIONALE=<rationale> APOLYSIS_REQUIRE_F6_MANAGED_MESH_DECISION=1 ./scripts/test-f6-managed-mesh-decision.sh",
    },
...
if require_ready and not managed_mesh_decision_ready:
    print(f"apolysis-f6: managed mesh decision failed closed ({report_path})", file=sys.stderr)
    print("missing requirements: " + ", ".join(report["missing_requirements"]), file=sys.stderr)
    raise SystemExit(1)
PY

cat <<EOF
apolysis-f6: managed mesh decision audit written ($output_dir)
APOLYSIS_F6_MANAGED_MESH_DECISION_REPORT=$report
EOF
