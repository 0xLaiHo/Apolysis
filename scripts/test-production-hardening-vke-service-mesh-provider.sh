...
confirm="${APOLYSIS_CONFIRM_PRODUCTION_HARDENING_VKE_SERVICE_MESH_PROVIDER:-0}"
default_kubeconfig="/home/mactavish/vultr-k8s/vke-a88389c3-f720-412d-9579-c83d3c21eabb.yaml"

if [[ "$confirm" != "1" ]]; then
    cat >&2 <<EOF
apolysis-production-hardening: Vultr VKE service-mesh provider qualification is opt-in.
Set APOLYSIS_CONFIRM_PRODUCTION_HARDENING_VKE_SERVICE_MESH_PROVIDER=1 after confirming the
...
if [[ ! -f "$KUBECONFIG" ]]; then
    echo "apolysis-production-hardening: KUBECONFIG does not exist: $KUBECONFIG" >&2
    exit 2
fi

mkdir -p "$repo_root/target"
output_dir="${APOLYSIS_PRODUCTION_HARDENING_VKE_SERVICE_MESH_PROVIDER_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/production-hardening-vke-service-mesh-provider.XXXXXX")}"
mkdir -p "$output_dir"
...
APOLYSIS_PRODUCTION_HARDENING_VKE_CLUSTER_READINESS_OUTPUT_DIR="$vke_readiness_dir" \
    "$repo_root/scripts/test-production-hardening-vke-cluster-readiness.sh" >/dev/null

APOLYSIS_CONFIRM_PRODUCTION_HARDENING_SERVICE_MESH_LIVE=1 \
APOLYSIS_PRODUCTION_HARDENING_SERVICE_MESH_LIVE_OUTPUT_DIR="$istio_live_dir" \
    "$repo_root/scripts/test-production-hardening-service-mesh-live-istio.sh" >/dev/null

vke_evidence="$vke_readiness_dir/apolysis-production-hardening-vke-cluster-readiness-evidence.json"
vke_report="$vke_readiness_dir/apolysis-production-hardening-vke-cluster-readiness-report.json"
istio_evidence="$istio_live_dir/apolysis-production-hardening-istio-live-evidence.json"
istio_report="$istio_live_dir/apolysis-production-hardening-istio-live-evidence-report.json"
evidence="$output_dir/apolysis-production-hardening-vke-service-mesh-provider-evidence.json"
report="$output_dir/apolysis-production-hardening-vke-service-mesh-provider-report.json"
...
evidence = {
    "schema_version": 1,
    "evidence_id": f"production-hardening-vke-service-mesh-provider-{observed_at_unix_ms}",
    "phase": "production-hardening.vke-service-mesh-provider",
    ...
    "vke_cluster_readiness_evidence_ref": "vke-cluster-readiness/apolysis-production-hardening-vke-cluster-readiness-evidence.json",
    "vke_cluster_readiness_report_ref": "vke-cluster-readiness/apolysis-production-hardening-vke-cluster-readiness-report.json",
    "istio_live_evidence_ref": "istio-live/apolysis-production-hardening-istio-live-evidence.json",
    "istio_live_report_ref": "istio-live/apolysis-production-hardening-istio-live-evidence-report.json",
    "live_provider": True,
...
cat <<EOF
apolysis-production-hardening: Vultr VKE service-mesh provider qualification passed ($output_dir)
APOLYSIS_PRODUCTION_HARDENING_MANAGED_MESH_PROVIDER=vultr_vke_istio
APOLYSIS_PRODUCTION_HARDENING_MANAGED_MESH_CONTROL_PLANE=$(jq -r '.provider_control_plane' "$evidence")
APOLYSIS_PRODUCTION_HARDENING_MANAGED_MESH_EVIDENCE=$evidence
APOLYSIS_PRODUCTION_HARDENING_MANAGED_MESH_REPORT=$report
EOF
