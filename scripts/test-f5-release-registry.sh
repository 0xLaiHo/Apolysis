...
output_dir="${APOLYSIS_F5_RELEASE_REGISTRY_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/f5-release-registry.XXXXXX")}"
mkdir -p "$output_dir"
output_dir="$(cd "$output_dir" && pwd)"
bundle_dir="$output_dir/release-bundle"
archive_root="$output_dir/immutable-archive"
archive_objects="$archive_root/objects"
registry_name="${APOLYSIS_F5_REGISTRY_NAME:-apolysis-f5-registry-$stamp}"
registry_repository="${APOLYSIS_F5_REGISTRY_REPOSITORY:-apolysisd}"
registry_tag="${APOLYSIS_F5_REGISTRY_TAG:-f5-registry-$stamp}"
...
registry_port="${APOLYSIS_F5_REGISTRY_PORT:-$(choose_free_port)}"
...
if ! curl -fsS "$registry_url/v2/" >/dev/null 2>&1; then
    echo "apolysis-f5: local OCI registry did not become ready" >&2
    docker logs "$registry_name" >&2 || true
    exit 1
fi

APOLYSIS_F5_RELEASE_OUTPUT_DIR="$bundle_dir" \
APOLYSIS_F5_RELEASE_IMAGE="$image" \
APOLYSIS_F5_REMOVE_IMAGE=0 \
    "$repo_root/scripts/build-f5-release-bundle.sh"

(
    cd "$bundle_dir"
    sha256sum -c apolysis-f5-release-checksums.sha256
)

docker push "$image"

curl -fsSL "$registry_url/v2/_catalog" \
    -o "$output_dir/apolysis-f5-registry-catalog.json"
curl -fsSL "$registry_url/v2/$registry_repository/tags/list" \
    -o "$output_dir/apolysis-f5-registry-tags-before-sbom.json"

image_manifest_body="$output_dir/apolysis-f5-registry-image-manifest.json"
image_manifest_headers="$output_dir/apolysis-f5-registry-image-manifest.headers"
curl -fsSL \
...
if [[ ! "$image_digest" =~ ^sha256:[0-9a-f]{64}$ ]]; then
    echo "apolysis-f5: local registry did not return a valid image digest" >&2
    exit 1
fi
...
cosign attach sbom \
    --type cyclonedx \
    --input-format json \
    --sbom "$bundle_dir/apolysis-f5-sbom.cdx.json" \
    --allow-http-registry \
...
curl -fsSL "$registry_url/v2/$registry_repository/tags/list" \
    -o "$output_dir/apolysis-f5-registry-tags-after-sbom.json"
jq -e --arg tag "$registry_tag" --arg sbom_tag "$sbom_tag" '
  (.tags | index($tag)) and (.tags | index($sbom_tag))
' "$output_dir/apolysis-f5-registry-tags-after-sbom.json" >/dev/null

sbom_manifest_body="$output_dir/apolysis-f5-registry-sbom-manifest.json"
sbom_manifest_headers="$output_dir/apolysis-f5-registry-sbom-manifest.headers"
curl -fsSL \
...
if [[ ! "$sbom_attachment_digest" =~ ^sha256:[0-9a-f]{64}$ ]]; then
    echo "apolysis-f5: local registry did not return a valid SBOM attachment digest" >&2
    exit 1
fi
...
data = {
    ...
    "releaseArtifacts": {
        "manifest": {
            ...
            "sha256": sha256(bundle_dir / "apolysis-f5-release-manifest.json"),
        },
        "provenance": {
            "path": "release-bundle/apolysis-f5-provenance.intoto.json",
            "sha256": sha256(bundle_dir / "apolysis-f5-provenance.intoto.json"),
        },
        "sbom": {
            "path": "release-bundle/apolysis-f5-sbom.cdx.json",
            "sha256": sha256(bundle_dir / "apolysis-f5-sbom.cdx.json"),
        },
        "checksums": {
            "path": "release-bundle/apolysis-f5-release-checksums.sha256",
            "sha256": sha256(bundle_dir / "apolysis-f5-release-checksums.sha256"),
        },
    },
    "evidenceFiles": {
        "catalog": "apolysis-f5-registry-catalog.json",
        "tagsBeforeSbom": "apolysis-f5-registry-tags-before-sbom.json",
        "tagsAfterSbom": "apolysis-f5-registry-tags-after-sbom.json",
        "imageManifest": "apolysis-f5-registry-image-manifest.json",
        "sbomManifest": "apolysis-f5-registry-sbom-manifest.json",
    },
    "registryObservedState": {
        "catalog": read_json(output_dir / "apolysis-f5-registry-catalog.json"),
        "tagsAfterSbom": read_json(output_dir / "apolysis-f5-registry-tags-after-sbom.json"),
    },
}
...
release_manifest_digest="$(sha256_file "$bundle_dir/apolysis-f5-release-manifest.json")"
...
if [[ -e "$archive_dir" ]]; then
    echo "apolysis-f5: immutable archive object already exists: $archive_dir" >&2
    exit 1
fi
mkdir -p "$archive_tmp"

for artifact in \
    "$bundle_dir/apolysis-f5-release-payload.tar.gz" \
    "$bundle_dir/apolysis-f5-apolysisd-image.tar" \
    "$bundle_dir/apolysis-f5-sbom.cdx.json" \
    "$bundle_dir/apolysis-f5-vulnerability-scan.json" \
    "$bundle_dir/apolysis-f5-provenance.intoto.json" \
    "$bundle_dir/apolysis-f5-release-manifest.json" \
    "$bundle_dir/apolysis-f5-release.pub" \
    "$bundle_dir/apolysis-f5-release-manifest.sigstore.json" \
    "$bundle_dir/apolysis-f5-provenance.sigstore.json" \
    "$bundle_dir/apolysis-f5-release-checksums.sha256" \
...
if [[ "$mutation_probe" != "denied" && "$EUID" -ne 0 ]]; then
    echo "apolysis-f5: immutable archive mutation probe unexpectedly succeeded" >&2
    exit 1
fi
...
jq -e --arg release_manifest_digest "$release_manifest_digest" --arg registry_attachment_digest "$registry_attachment_digest" '
  ...
  and ([.artifacts[].path] | index("apolysis-f5-registry-attachment.json"))
  and ([.artifacts[].path] | index("apolysis-f5-release-manifest.json"))
  and ([.artifacts[].path] | index("apolysis-f5-apolysisd-image.tar"))
...
printf 'apolysis-f5: release registry/archive gate passed (%s)\n' "$output_dir"
