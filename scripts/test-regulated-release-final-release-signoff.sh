...
output_dir="${APOLYSIS_REGULATED_RELEASE_FINAL_RELEASE_SIGNOFF_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/regulated-release-final-release-signoff.XXXXXX")}"
mkdir -p "$output_dir"
output_dir="$(cd "$output_dir" && pwd)"

report="$output_dir/apolysis-regulated-release-final-release-signoff-report.json"
require_ready="${APOLYSIS_REQUIRE_REGULATED_RELEASE_FINAL_RELEASE_SIGNOFF:-0}"
...
manifest_path = output_dir / "apolysis-regulated-release-final-release-signoff-manifest.json"
generated_signoff_path = output_dir / "apolysis-regulated-release-final-release-signoff.json"
...
source_report_value = env_value(
    "APOLYSIS_REGULATED_RELEASE_SOURCE_REPORT",
...
if source_report_path is None:
    missing_requirements.append("APOLYSIS_REGULATED_RELEASE_REPORT")
...
if source_report_path is not None and source_report_path.is_file():
    if source_phase not in {"regulated-release.live-provider-readback", "regulated-release.final-release-signoff"}:
...
if provided_signoff_path is not None:
    ...
    approver = env_value("APOLYSIS_REGULATED_RELEASE_FINAL_SIGNOFF_APPROVER", "APOLYSIS_REGULATED_RELEASE_FINAL_RELEASE_APPROVER")
    decision = env_value("APOLYSIS_REGULATED_RELEASE_FINAL_SIGNOFF_DECISION", "APOLYSIS_REGULATED_RELEASE_FINAL_RELEASE_DECISION")
    rationale = env_value("APOLYSIS_REGULATED_RELEASE_FINAL_SIGNOFF_RATIONALE", "APOLYSIS_REGULATED_RELEASE_FINAL_RELEASE_RATIONALE")
    approved_at = env_value("APOLYSIS_REGULATED_RELEASE_FINAL_SIGNOFF_APPROVED_AT", "APOLYSIS_REGULATED_RELEASE_FINAL_RELEASE_APPROVED_AT")
...
if phase and phase != "regulated-release.final-release-signoff":
    missing_requirements.append("final_release_signoff_phase")
elif not phase:
    missing_requirements.append("final_release_signoff_phase")

if release_scope and release_scope not in {"regulated_release", "regulated_release"}:
...
manifest = {
    "schema_version": 1,
    "phase": "regulated-release.final-release-signoff",
    "source": "regulated_release_final_release_signoff",
...
report = {
    ...
    "notes": [
        ...
        "regulated-release.final-release-signoff validates the final regulated-release sign-off artifact against the retained RegulatedRelease aggregate evidence.",
...
if require_ready and not final_release_signoff_ready:
    print(f"apolysis-regulated_release: final release sign-off failed closed ({report_path})", file=sys.stderr)
    print("missing requirements: " + ", ".join(report["missing_requirements"]), file=sys.stderr)
    raise SystemExit(1)
PY

cat <<EOF
apolysis-regulated_release: final release sign-off audit written ($output_dir)
APOLYSIS_REGULATED_RELEASE_FINAL_RELEASE_SIGNOFF_REPORT=$report
EOF
