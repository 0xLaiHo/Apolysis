...
output_dir="${APOLYSIS_PRODUCTION_HARDENING_FINAL_PROVIDER_COMPLETION_OUTPUT_DIR:-$(mktemp -d "$repo_root/target/production-hardening-final-provider-completion.XXXXXX")}"
mkdir -p "$output_dir"
output_dir="$(cd "$output_dir" && pwd)"

report="$output_dir/apolysis-production-hardening-final-provider-completion-report.json"
...
readiness_report = Path(readiness_dir) / "apolysis-production-hardening-final-provider-readiness-report.json"
bundle_env_report = Path(bundle_env_dir) / "apolysis-production-hardening-final-provider-bundle-env-report.json"
...
match = re.search(r"apolysis-production-hardening: final external provider bundle passed \(([^)]+)\)", final_bundle_output)
if match:
...
if final_bundle_output_dir:
    final_bundle_report_path = Path(final_bundle_output_dir) / "apolysis-production-hardening-final-external-provider-bundle-report.json"
    final_bundle_bundle_path = (
        Path(final_bundle_output_dir)
        / "bundle-root"
        / "apolysis-production-hardening-final-external-provider-bundle.json"
    )
...
APOLYSIS_PRODUCTION_HARDENING_FINAL_PROVIDER_READINESS_OUTPUT_DIR="$readiness_dir" \
...
if [[ "$readiness_rc" -ne 0 ]]; then
    ...
    echo "apolysis-production-hardening: final provider completion failed at readiness ($report)" >&2
    cat "$output_dir/final-provider-readiness.err" >&2
    exit "$readiness_rc"
fi

set +e
APOLYSIS_REQUIRE_PRODUCTION_HARDENING_FINAL_BUNDLE_ENV=1 \
APOLYSIS_RUN_PRODUCTION_HARDENING_FINAL_BUNDLE=1 \
APOLYSIS_PRODUCTION_HARDENING_FINAL_PROVIDER_BUNDLE_ENV_OUTPUT_DIR="$bundle_env_dir" \
    "$repo_root/scripts/prepare-production-hardening-final-provider-bundle-env.sh" \
...
if [[ "$bundle_env_rc" -ne 0 ]]; then
    ...
    echo "apolysis-production-hardening: final provider completion failed at final bundle assembly ($report)" >&2
...
cat <<EOF
apolysis-production-hardening: final provider completion gate passed ($output_dir)
APOLYSIS_PRODUCTION_HARDENING_FINAL_PROVIDER_COMPLETION_REPORT=$report
EOF
